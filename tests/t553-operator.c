...
int main(int argc, char **argv) {
  Ceed ceed;
  CeedElemRestriction elem_restr_x, elem_restr_qd,
  ...
  CeedElemRestrictionCreate(ceed, num_elem, P_c, 1, 1, num_dofs_u_c,
                            CEED_MEM_HOST,
                            CEED_USE_POINTER, ind_u_c, &elem_restr_u_c);
  ...
  CeedElemRestrictionCreate(ceed, num_elem, P_f, 1, 1, num_dofs_u_f,
                            CEED_MEM_HOST,
                            CEED_USE_POINTER, ind_u_f, &elem_restr_u_f);
  ...
  CeedOperatorSetField(op_setup, "qdata", elem_restr_qd, CEED_BASIS_COLLOCATED,
                       CEED_VECTOR_ACTIVE);

  CeedOperatorSetField(op_mass_f, "qdata", elem_restr_qd, CEED_BASIS_COLLOCATED,
                       q_data);
  CeedOperatorSetField(op_mass_f, "u", elem_restr_u_f, basis_u,
                       CEED_VECTOR_ACTIVE);
  CeedOperatorSetField(op_mass_f, "v", elem_restr_u_f, basis_u,
                       CEED_VECTOR_ACTIVE);
  ...
  CeedBasisCreateTensorH1Lagrange(ceed, 1, 1, P_c, P_f, CEED_GAUSS_LOBATTO,
                                  &basis_c_to_f);
  const CeedScalar *interp_c_to_f;
  CeedBasisGetInterp1D(basis_c_to_f, &interp_c_to_f);
  CeedOperatorMultigridLevelCreateH1(op_mass_f, p_mult_f, elem_restr_u_c,
                                     basis_u_c, interp_c_to_f, &op_mass_c,
                                     &op_prolong, &op_restrict);
