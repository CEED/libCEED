...
int main(int argc, char **argv) {
  ...
  CeedVectorCreate(ceed, num_dofs_u_fine, &p_mult_fine);
  CeedVectorCreate(ceed, num_dofs_u_coarse, &u_coarse);
  CeedVectorCreate(ceed, num_dofs_u_coarse, &v_coarse);
  CeedVectorCreate(ceed, num_dofs_u_fine, &u_fine);
  CeedVectorCreate(ceed, num_dofs_u_fine, &v_fine);
  ...
  CeedBasisCreateTensorH1Lagrange(ceed, 1, 1, p_fine, q, CEED_GAUSS, &basis_u);
  ...
  CeedBasisCreateTensorH1Lagrange(ceed, 1, 1, p_coarse, p_fine, CEED_GAUSS_LOBATTO, &basis_coarse_to_fine);
  const CeedScalar *interp_coarse_to_fine;
  CeedBasisGetInterp1D(basis_coarse_to_fine, &interp_coarse_to_fine);
  CeedOperatorMultigridLevelCreateH1(op_mass_fine, p_mult_fine, elem_restriction_u_coarse, basis_u_coarse, interp_coarse_to_fine, &op_mass_coarse,
  ...
  // Check output
  {
    ...
    for (CeedInt i = 0; i < num_dofs_u_coarse; i++) {
      sum += v_array[i];
    }
