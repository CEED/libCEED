...
int main(int argc, char **argv) {
  ...
  CeedElemRestrictionCreateStrided(ceed, num_elem_tet, Q_tet, dim * (dim + 1) / 2, dim * (dim + 1) / 2 * num_qpts_tet, strides_qd_tet,
                                   &elem_restr_qd_i_tet);
  ...
  CeedOperatorSetField(op_diff_tet, "rho", elem_restr_qd_i_tet, CEED_BASIS_COLLOCATED, q_data_tet);
  CeedOperatorSetField(op_diff_tet, "u", elem_restr_u_tet, basis_u_tet, CEED_VECTOR_ACTIVE);
  CeedOperatorSetField(op_diff_tet, "v", elem_restr_u_tet, basis_u_tet, CEED_VECTOR_ACTIVE);
  ...
  CeedElemRestrictionCreateStrided(ceed, num_elem_hex, Q_hex * Q_hex, dim * (dim + 1) / 2, dim * (dim + 1) / 2 * num_qpts_hex, strides_qd_hex,
                                   &elem_restr_qd_i_hex);
  ...
  CeedOperatorSetField(op_diff_hex, "rho", elem_restr_qd_i_hex, CEED_BASIS_COLLOCATED, q_data_hex);
  CeedOperatorSetField(op_diff_hex, "u", elem_restr_u_hex, basis_u_hex, CEED_VECTOR_ACTIVE);
  CeedOperatorSetField(op_diff_hex, "v", elem_restr_u_hex, basis_u_hex, CEED_VECTOR_ACTIVE);
