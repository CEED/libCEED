...
int main(int argc, char **argv) {
  ...
  CeedOperatorLinearAssemblePointBlockDiagonal(op_mass, A, CEED_REQUEST_IMMEDIATE);
  ...
  for (int i = 0; i < num_dofs; i++) {
    for (int j = 0; j < num_comp; j++) {
      ...
      for (int k = 0; k < num_comp; k++) assembled_true[i * num_comp * num_comp + k * num_comp + j] += v[i + k * num_dofs];
