...
int main(int argc, char **argv) {
  ...
  CeedBasis           basis_x, basis_temp, basis_c, basis_f;
  CeedQFunction       qf_setup, qf_mass;
  CeedOperator        op_setup, op_mass_c, op_mass_f, op_prolong, op_restrict;
  CeedVector          q_data, X, U_c, U_f, V_c, V_f, p_mult_fine;
  const CeedScalar   *hv;
  ...
  CeedBasisCreateTensorH1Lagrange(ceed, 1, num_comp, P_c, Q, CEED_GAUSS, &basis_temp);
  const CeedScalar *interp, *grad, *q_ref, *q_weight;
  CeedBasisGetInterp1D(basis_temp, &interp);
  CeedBasisGetGrad1D(basis_temp, &grad);
  CeedBasisGetQRef(basis_temp, &q_ref);
  CeedBasisGetQWeights(basis_temp, &q_weight);
  CeedBasisCreateH1(ceed, CEED_TOPOLOGY_LINE, num_comp, P_c, Q, interp, grad, q_ref, q_weight, &basis_c);
  CeedBasisDestroy(&basis_temp);
  CeedBasisCreateTensorH1Lagrange(ceed, 1, num_comp, P_f, Q, CEED_GAUSS, &basis_temp);
  CeedBasisGetInterp1D(basis_temp, &interp);
  CeedBasisGetGrad1D(basis_temp, &grad);
  CeedBasisGetQRef(basis_temp, &q_ref);
  CeedBasisGetQWeights(basis_temp, &q_weight);
  CeedBasisCreateH1(ceed, CEED_TOPOLOGY_LINE, num_comp, P_f, Q, interp, grad, q_ref, q_weight, &basis_f);
  CeedBasisDestroy(&basis_temp);
  ...
  CeedOperatorSetField(op_mass_f, "u", elem_restr_u_f, basis_f, CEED_VECTOR_ACTIVE);
  CeedOperatorSetField(op_mass_f, "v", elem_restr_u_f, basis_f, CEED_VECTOR_ACTIVE);
  ...
  CeedOperatorMultigridLevelCreate(op_mass_f, p_mult_fine, elem_restr_u_c, basis_c, &op_mass_c, &op_prolong, &op_restrict);
  ...
  CeedBasisDestroy(&basis_c);
  CeedBasisDestroy(&basis_f);
