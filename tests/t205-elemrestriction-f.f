!-----------------------------------------------------------------------
      ...
      call ceedelemrestrictionapply(r,ceed_notranspose,ceed_transpose,x,y,ceed_request_immediate,err)
      ...
      do i=0,ne-1
        do n=1,2
          ...
          if (abs(diff) > 1.0D-15) then
            write(*,*) 'Error in restricted array y(',i*4+n,')=',yy(i*4+n+offset),'!=',10+(2*i+n)/2
          endif
          diff=20+(2*i+n)/2-yy(i*4+n+2+offset)
          if (abs(diff) > 1.0D-15) then
            write(*,*) 'Error in restricted array y(',i*4+n+2,')=',yy(i*4+n+2+offset),'!=',20+(2*i+n)/2
          endif
        enddo
      enddo
