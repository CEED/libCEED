...
int main(int argc, char **argv) {
  ...
  CeedScalar          assembled_true_oriented[num_dofs * num_dofs];
  CeedScalar          assembled_true_curl_oriented[num_dofs * num_dofs];
  ...
  CeedOperatorSetField(op_mass_oriented, "rho", elem_restriction_q_data, CEED_BASIS_COLLOCATED, q_data);
  ...
  CeedOperatorSetField(op_mass_curl_oriented, "rho", elem_restriction_q_data, CEED_BASIS_COLLOCATED, q_data);
  ...
  for (CeedInt k = 0; k < num_dofs * num_dofs; ++k) {
    ...
    assembled_true_oriented[k]        = 0.0;
    assembled_true_curl_oriented[k]   = 0.0;
  }
  ...
  for (CeedInt i = 0; i < num_dofs; i++) {
    ...
    for (CeedInt k = 0; k < num_dofs; k++) assembled_true_oriented[i * num_dofs + k] = v_array[k];
  ...
  for (CeedInt i = 0; i < num_dofs; i++) {
    ...
    CeedVectorGetArrayRead(v, CEED_MEM_HOST, &v_array);
    for (CeedInt k = 0; k < num_dofs; k++) assembled_true_curl_oriented[i * num_dofs + k] = v_array[k];
    CeedVectorRestoreArrayRead(v, &v_array);
  }
  ...
  for (CeedInt i = 0; i < num_dofs; i++) {
    for (CeedInt j = 0; j < num_dofs; j++) {
      ...
      if (fabs(assembled_true_oriented[j * num_dofs + i] - assembled_values[j * num_dofs + i]) > 100. * CEED_EPSILON) {
        // LCOV_EXCL_START
        printf("[%" CeedInt_FMT ", %" CeedInt_FMT "] Error in oriented manual assembly: %f != %f\n", i, j, assembled_true_oriented[j * num_dofs + i],
               assembled_values[j * num_dofs + i]);
        // LCOV_EXCL_STOP
      }
      ...
      if (fabs(assembled_true_curl_oriented[j * num_dofs + i] - assembled_values[j * num_dofs + i]) > 100. * CEED_EPSILON) {
        // LCOV_EXCL_START
        printf("[%" CeedInt_FMT ", %" CeedInt_FMT "] Error in curl-oriented manual assembly: %f != %f\n", i, j,
               assembled_true_curl_oriented[j * num_dofs + i], assembled_values[j * num_dofs + i]);
        // LCOV_EXCL_STOP
      }
    }
  }
