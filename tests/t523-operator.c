...
int main(int argc, char **argv) {
  ...
  CeedElemRestrictionCreateStrided(ceed,  nelem_tet, Q_tet, 1, num_qpts_tet,
                                   strides_qd_tet, &elem_restr_qd_tet);

  // -- Bases
  buildmats(q_ref, q_weight, interp, grad);
  CeedBasisCreateH1(ceed, CEED_FE_TOPO_TRIANGLE, dim, P_tet, Q_tet, interp, grad,
                    q_ref, q_weight, &basis_x_tet);

  buildmats(q_ref, q_weight, interp, grad);
  CeedBasisCreateH1(ceed, CEED_FE_TOPO_TRIANGLE, 1, P_tet, Q_tet, interp, grad,
  ...
  CeedOperatorSetField(op_setup_tet, "dx", elem_restr_x_tet, basis_x_tet,
                       CEED_VECTOR_ACTIVE);
  CeedOperatorSetField(op_setup_tet, "rho", elem_restr_qd_tet,
