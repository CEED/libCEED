...
int main(int argc, char **argv) {
  ...
  CeedOperatorSetField(op_mass_f, "qdata", elem_restr_qd, CEED_BASIS_COLLOCATED,
                       q_data);
  CeedOperatorSetField(op_mass_f, "u", elem_restr_c_f, basis_u,
                       CEED_VECTOR_ACTIVE);
  CeedOperatorSetField(op_mass_f, "v", elem_restr_c_f, basis_u,
                       CEED_VECTOR_ACTIVE);
  ...
  CeedBasisCreateTensorH1Lagrange(ceed, 1, num_comp, P_c, Q, CEED_GAUSS,
                                  &basis_u_c);
  CeedBasisCreateTensorH1Lagrange(ceed, 1, num_comp, P_c, P_f,
                                  CEED_GAUSS_LOBATTO, &basis_c_to_f);
  const CeedScalar *interp_c_to_f;
  CeedBasisGetInterp1D(basis_c_to_f, &interp_c_to_f);
  CeedOperatorMultigridLevelCreateTensorH1(op_mass_f, p_mult_f,
      elem_restr_u_c, basis_u_c, interp_c_to_f, &op_mass_c, &op_prolong,
      &op_restrict);
  ...
  CeedElemRestrictionDestroy(&elem_restr_c_f);
