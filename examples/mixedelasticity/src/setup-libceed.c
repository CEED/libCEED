...
PetscErrorCode SetupLibceed(DM dm, Ceed ceed, AppCtx app_ctx, ProblemData problem_data, CeedData ceed_data, CeedVector rhs_ceed) {
  ...
  CeedQFunction      qf_setup_geo, qf_setup_rhs, qf_residual, qf_error;
  CeedOperator       op_setup_geo, op_setup_rhs, op_residual, op_error;
  ...
  // CEED restrictions
  PetscCall(CreateRestrictionFromPlex(ceed, dm_coord, 0, 0, 0, &ceed_data->elem_restr_x));
  PetscCall(CreateRestrictionFromPlex(ceed, dm, 0, 0, 0, &ceed_data->elem_restr_u));

  PetscCall(DMPlexGetHeightStratum(dm, 0, &c_start, &c_end));
  num_elem = c_end - c_start;
  CeedBasisGetNumQuadraturePoints(ceed_data->basis_u, &num_qpts);

  CeedElemRestrictionCreateStrided(ceed, num_elem, num_qpts, num_comp_u, num_comp_u * num_elem * num_qpts, CEED_STRIDES_BACKEND,
  ...
  CeedOperatorSetField(op_error, "qdata", ceed_data->elem_restr_qdata, CEED_BASIS_COLLOCATED, ceed_data->q_data);
