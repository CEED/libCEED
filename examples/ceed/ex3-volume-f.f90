...
!                             libCEED Example 3
!
! This example illustrates a simple usage of libCEED to compute the volume of a 3D body using
! matrix-free application of a mass operator. This example also uses a diffusion operator, which
! provides zero contribution to the computed volume but demonstrates libCEED's ability to handle
! multiple basis evaluation modes for the same input and output vectors.
...
!
!     ./ex3-volume-f
!     ./ex3-volume-f -ceed /cpu/self
!     ./ex3-volume-f -ceed /gpu/cuda
!
! Test in 1D-3D
...
!> @file
!> libCEED example using mass + diffusion operator to compute volume

    include 'ex3-volume-f.h'
...
program main
    ...
    external build_mass_diff, apply_mass_diff
...
! Build CeedElemRestriction objects describing the mesh and solution discrete representations
    ...
    call buildcartesianrestriction(ceed, fe_dim, num_xyz, sol_degree, 1 + fe_dim * (fe_dim + 1) / 2, sol_size, num_qpts,&
...
! Create the QFunction that builds the mass + diffusion operator (i.e. computes its quadrature data) and set its context data
    call ceedqfunctioncreateinterior(ceed, 1, build_mass_diff,&
             &SOURCE_DIR&
             &//'ex3-volume-f-c.h:build_mass_diff'//char(0), qf_build, err)
    ...
    call ceedqfunctionaddoutput(qf_build, 'qdata', 1 + fe_dim * (fe_dim + 1) / 2, ceed_eval_none, err)
...
! Compute the quadrature data for the mass + diffusion operator
    ...
    call ceedvectorcreate(ceed, num_elem * elem_qpts * (1 + fe_dim * (fe_dim + 1) / 2), q_data, err)
...
! Create the QFunction that defines the action of the mass + diffusion operator
    call ceedqfunctioncreateinterior(ceed, 1, apply_mass_diff,&
             &SOURCE_DIR&
             &//'ex3-volume-f-c.h:apply_mass_diff'//char(0), qf_apply, err)
    ...
    call ceedqfunctionaddinput(qf_apply, 'qdata', 1 + fe_dim * (fe_dim + 1) / 2, ceed_eval_none, err)
...
! Compute and print the sum of the entries of 'v' giving the mesh volume
    ...
    do i = 1, sol_size
      computed_volume = computed_volume + v_array(i)
    end do
...
! LCOV_EXCL_START
      ...
      write (*, *) 'Volume error         :', (computed_volume - exact_volume)
! LCOV_EXCL_STOP
    else
      if (fe_dim == 1) then
        if (abs(computed_volume - exact_volume) > 200.d0 * 1e-15) then
! LCOV_EXCL_START
          write (*, *) 'Volume error : ', (computed_volume - exact_volume)
! LCOV_EXCL_STOP
        end if
      else
        if (abs(computed_volume - exact_volume) > 1e-5) then
! LCOV_EXCL_START
          write (*, *) 'Volume error : ', (computed_volume - exact_volume)
! LCOV_EXCL_STOP
        end if
