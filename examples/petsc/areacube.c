...
// Build with:
//
//     make areacube [PETSC_DIR=</path/to/petsc>] [CEED_DIR=</path/to/libceed>]
//
// Sample runs:
//   Sequential:
//
//     areacube -petscspace_degree 3
//
//   In parallel:
//
...
/// @file
/// libCEED example using the mass operator to compute a cube surface area using PETSc with DMPlex
...
int main(int argc, char **argv) {
  ...
  if (!test_mode) {
    ierr = PetscPrintf(comm,
                       "\n-- libCEED + PETSc Surface Area of a Cube --\n"
                       "  libCEED:\n"
                       "    libCEED Backend                    : %s\n"
                       "  Mesh:\n"
