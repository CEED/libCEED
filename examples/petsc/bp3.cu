...
extern "C" __global__ void Setup(void *ctx, CeedInt Q, CeedInt N,
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    qd[i+N*0] = qw * (A11*A11 + A12*A12 + A13*A13);
    qd[i+N*1] = qw * (A11*A21 + A12*A22 + A13*A23);
    qd[i+N*2] = qw * (A11*A31 + A12*A32 + A13*A33);
    qd[i+N*3] = qw * (A21*A21 + A22*A22 + A23*A23);
    qd[i+N*4] = qw * (A21*A31 + A22*A32 + A23*A33);
    qd[i+N*5] = qw * (A31*A31 + A32*A32 + A33*A33);
    const CeedScalar c[3] = { 0, 1., 2. };
    const CeedScalar k[3] = { 1., 2., 3. };
    true_soln[i] = sin(M_PI*(c[0] + k[0]*x[i+N*0])) *
                   sin(M_PI*(c[1] + k[1]*x[i+N*1])) *
                   sin(M_PI*(c[2] + k[2]*x[i+N*2]));
...
// *****************************************************************************
extern "C" __global__ void Diff(void *ctx, CeedInt Q, CeedInt N,
                                CeedQFunctionArguments args) {
...
// *****************************************************************************
extern "C" __global__ void Error(void *ctx, CeedInt Q, CeedInt N,
                                 CeedQFunctionArguments args) {
