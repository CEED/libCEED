...
// This example is intentionally "raw", using only low-level communication primitives from PETSc
...
static int Setup(void *ctx, void *qdata, CeedInt Q,
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    rho[i] = det * w[i];
    target[i] = PetscSqrtScalar(PetscSqr(x[0][i]) + PetscSqr(x[1][i]) + PetscSqr(x[2][i]));
  }
  return 0;
}
...
static PetscErrorCode MatMult_Mass(Mat A, Vec X, Vec Y) {
  ...
  ierr = VecScatterBegin(user->ltog, X, user->Xloc, INSERT_VALUES, SCATTER_REVERSE);CHKERRQ(ierr);
  ierr = VecScatterEnd(user->ltog, X, user->Xloc, INSERT_VALUES, SCATTER_REVERSE);CHKERRQ(ierr);
  ...
  if (Y) {
    ...
    ierr = VecScatterBegin(user->ltog, user->Yloc, Y, ADD_VALUES, SCATTER_FORWARD); CHKERRQ(ierr);
    ierr = VecScatterEnd(user->ltog, user->Yloc, Y, ADD_VALUES, SCATTER_FORWARD); CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
...
int main(int argc, char **argv) {
  ...
  ierr = PetscOptionsString("-ceed_resource", "CEED resource specifier",
                            NULL, ceedresource, ceedresource,
                            sizeof(ceedresource), NULL); CHKERRQ(ierr);
  localdof = 1000;
  ierr = PetscOptionsInt("-local", "Target number of locally owned degrees of freedom per process",
  ...
  for (localelem = PetscMax(1, localdof / (degree*degree*degree)); ; localelem++) {
...

  {
    ...
    ierr = ISCreateGeneral(comm, lsize, ltogind, PETSC_OWN_POINTER, &ltogis); CHKERRQ(ierr);
    ierr = VecScatterCreate(Xloc, NULL, X, ltogis, &ltog); CHKERRQ(ierr); CHKERRQ(ierr);
  ...
  {
    CeedScalar *xloc;
    CeedInt shape[3] = {melem[0]+1, melem[1]+1, melem[2]+1}, len = shape[0]*shape[1]*shape[2];
    xloc = malloc(len*3*sizeof xloc[0]);
    for (CeedInt i=0; i<shape[0]; i++) {
      for (CeedInt j=0; j<shape[1]; j++) {
        for (CeedInt k=0; k<shape[2]; k++) {
          xloc[((i*shape[1]+j)*shape[2]+k) + 0*len] = 1.*(irank[0]*melem[0]+i) / (p[0]*melem[0]);
          xloc[((i*shape[1]+j)*shape[2]+k) + 1*len] = 1.*(irank[1]*melem[1]+j) / (p[1]*melem[1]);
          xloc[((i*shape[1]+j)*shape[2]+k) + 2*len] = 1.*(irank[2]*melem[2]+k) / (p[2]*melem[2]);
        }
      }
    }
  ...
  ierr = MatShellSetOperation(mat, MATOP_MULT, (void(*)(void))MatMult_Mass); CHKERRQ(ierr);
