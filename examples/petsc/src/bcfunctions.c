...
#define M_PI 3.14159265358979323846
  ...
  for (PetscInt i = 0; i < num_comp_u; i++)
    u[i] = sin(M_PI * (c[0] + k[0] * x[0])) * sin(M_PI * (c[1] + k[1] * x[1])) * sin(M_PI * (c[2] + k[2] * x[2]));
...
PetscErrorCode BCsMass(PetscInt dim, PetscReal time, const PetscReal x[], PetscInt num_comp_u, PetscScalar *u, void *ctx) {
  ...
  for (PetscInt i = 0; i < num_comp_u; i++) u[i] = PetscSqrtScalar(PetscSqr(x[0]) + PetscSqr(x[1]) + PetscSqr(x[2]));
