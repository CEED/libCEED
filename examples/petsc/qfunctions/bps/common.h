...
CEED_QFUNCTION(Error)(void *ctx, CeedInt Q,
  ...
  for (CeedInt i=0; i<Q; i++) {
    error[i] = (u[i] - target[i])*q_data[i]*q_data[i];
  }
  return 0;
}
...
CEED_QFUNCTION(Error3)(void *ctx, CeedInt Q,
  ...
  CeedScalar *error = out[0];
  for (CeedInt i=0; i<Q; i++) {
    error[i+0*Q] = (u[i+0*Q] - target[i+0*Q])*q_data[i]*q_data[i];
    error[i+1*Q] = (u[i+1*Q] - target[i+1*Q])*q_data[i]*q_data[i];
    error[i+2*Q] = (u[i+2*Q] - target[i+2*Q])*q_data[i]*q_data[i];
  }
  return 0;
}
// -----------------------------------------------------------------------------
