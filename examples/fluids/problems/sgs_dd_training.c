...
static PetscErrorCode SGS_DD_TrainingSetupNodalInputEvaluation(Ceed ceed, User user, CeedData ceed_data,
  ...
  PetscCall(GetRestrictionForDomain(ceed, sgs_dd_train->dm_dd_inputs, 0, 0, 0, num_qpts_1d, 0, &elem_restr_sgs, NULL, NULL));
  ...
  switch (user->phys->state_var) {
    ...
    case STATEVAR_CONSERVATIVE:
      CeedQFunctionCreateInterior(ceed, 1, ComputeSGS_DDAnisotropicInputsNodal_Conserv, ComputeSGS_DDAnisotropicInputsNodal_Conserv_loc,
                                  &qf_sgs_dd_nodal);
      break;
    default:
