...
static PetscErrorCode SetupTrainingDataCalculation(Ceed ceed, User user, CeedData ceed_data, ProblemData *problem,
  ...
  PetscCallCeed(ceed, CeedQFunctionAddInput(qf_sgs_dd_train, "x", num_comp_x, CEED_EVAL_INTERP));
  ...
  PetscCallCeed(ceed, CeedOperatorSetField(op_sgs_dd_train, "x", ceed_data->elem_restr_x, basis_x_to_q, ceed_data->x_coord));
