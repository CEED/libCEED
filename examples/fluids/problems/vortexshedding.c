...
PetscErrorCode NS_VORTEXSHEDDING(ProblemData *problem, DM dm, void *ctx, SimpleBC bc) {
  ...
  ierr = PetscCalloc1(1, &vortexshedding_ctx);
  CHKERRQ(ierr);
  ...
  PetscInt narray = 3;
  ierr            = PetscOptionsScalarArray("-freestream_velocity", "Velocity at freestream", NULL, U_infty, &narray, NULL);
  CHKERRQ(ierr);
  PetscCheck(narray == 3, comm, PETSC_ERR_ARG_SIZ, "-freestream_velocity should recieve array of size 3, instead recieved size %" PetscInt_FMT ".",
             narray);
  ierr = PetscOptionsScalar("-freestream_pressure", "Pressure at freestream", NULL, P0, &P0, NULL);
  CHKERRQ(ierr);
  ierr = PetscOptionsScalar("-freestream_temperature", "Temperature at freestream", NULL, T_infty, &T_infty, NULL);
  CHKERRQ(ierr);
  ierr = PetscOptionsScalar("-L", "Length of the rectangular channel", NULL, L, &L, NULL);
  CHKERRQ(ierr);
  ierr = PetscOptionsScalar("-H", "Heigth of the rectangular channel", NULL, H, &H, NULL);
  CHKERRQ(ierr);
  ierr = PetscOptionsScalar("-D", "Cylinder diameter", NULL, D, &D, NULL);
  CHKERRQ(ierr);
  PetscOptionsEnd();
  ...
  ierr = PetscCalloc1(1, &freestream_ctx);
  CHKERRQ(ierr);
  ...
  CeedQFunctionContextSetData(vortexshedding_context, CEED_MEM_HOST, CEED_USE_POINTER, sizeof(*vortexshedding_ctx), vortexshedding_ctx);
  CeedQFunctionContextSetDataDestroy(vortexshedding_context, CEED_MEM_HOST, FreeContextPetsc);
  CeedQFunctionContextDestroy(&problem->ics.qfunction_context);
