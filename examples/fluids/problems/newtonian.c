...
PetscErrorCode NS_NEWTONIAN_IG(ProblemData *problem, DM dm, void *ctx, SimpleBC bc) {
  ...
  PetscCall(PetscOptionsScalar("-ramp_amplitude", "Sponge amplitude", NULL, ramp_amplitude, &ramp_amplitude, NULL));
  PetscCall(PetscOptionsScalar("-ramp_start", "Sponge start", NULL, ramp_start, &ramp_start, NULL));
  PetscCall(PetscOptionsScalar("-ramp_length", "Sponge ramp length", NULL, ramp_length, &ramp_length, NULL));
  PetscOptionsEnd();
