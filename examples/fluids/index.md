...
This test case is taken from {cite}`shakib1991femcfd`. A cylinder with diameter $D=1$ is centered at $(0,0)$ in a computational domain $-4.5 \leq x \leq 15.5$, $-4.5 \leq y \leq 4.5$. The domain is filled with an ideal gas at rest (zero velocity) with temperature, pressure, viscosity assumed constant at $0.01$. At time $t=0$, it is subjected to freestream boundary conditions at the inflow (left) and outflow (right), with exterior reference state at velocity $(1, 0, 0)$ giving Reynolds number $100$ and Mach number $0.01$. On the top and bottom boundaries $(y = \pm 4.5)$, symmetry condition, zero normal velocity component, zero heat-flux are imposed. On the cylinder wall, a no-slip boundary condition, no heat-flux condition are imposed. As we evolve in time, eddies appear past the cylinder leading to a vortex shedding which is known as the vortex street.

To run this case, a mesh file `-cylinder.geo` is created with `-Gmsh`. We have graded the mesh for better resolution near the cylinder wall. This test case can be run with linear and quadratic elements on the mesh.
