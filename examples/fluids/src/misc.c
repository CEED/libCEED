...
PetscErrorCode GetError_NS(CeedData ceed_data, DM dm, User user, Vec Q, PetscScalar final_time) {
  ...
  PetscCall(VecStrideNormAll(Q_exact_loc, norm_type, norm_exact));
  PetscCallMPI(MPI_Allreduce(MPI_IN_PLACE, norm_exact, 5, MPIU_REAL, norm_reduce, PETSC_COMM_WORLD));
  PetscCall(VecAXPY(Q_loc, -1.0, Q_exact_loc));               // Q_loc -= Q_exact_loc
  PetscCall(VecStrideNormAll(Q_loc, norm_type, norm_error));  // norm(Q_loc)
  PetscCallMPI(MPI_Allreduce(MPI_IN_PLACE, norm_error, 5, MPIU_REAL, norm_reduce, PETSC_COMM_WORLD));

  // Report error
  PetscCall(PetscPrintf(PETSC_COMM_WORLD, "Relative Error in conservative:\n"));
  for (int i = 0; i < 5; i++) {
