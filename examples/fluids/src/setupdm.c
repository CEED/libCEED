...
PetscErrorCode SetUpDM(DM *dm, ProblemData problem, PetscInt degree, PetscInt q_extra, SimpleBC bc, Physics phys) {
  ...
  if (has_NL_vector) {
    ...
    if (SkipProjection) {
      ...
      PetscCall(PetscStrcpy(vecNamed, vecName));
      PetscCall(PetscStrlcat(vecNamed, "d", PETSC_MAX_PATH_LEN));
      PetscCall(DMHasNamedLocalVector(*dm, vecNamed, &has_NL_vectord));
      if (has_NL_vectord) PetscCall(PetscStrcpy(vecName, vecNamed));  // distributed correction is the vector we should use
      DM new_dm = NULL;
  ...
  {  // Add strong boundary conditions to DM
    ...
    for (PetscInt i = 0; i < problem->num_bc_defs; i++) {
      ...
      if (essential_comps > 0) {
        ...
        PetscCall(DMAddBoundary(*dm, DM_BC_ESSENTIAL, name, label, num_label_values, label_values, 0, num_essential_comps, essential_comps, NULL,
                                NULL, NULL, NULL));
      }
    }
    {
