...
PetscErrorCode DiffFluxProjectionInitialize(User user, CeedElemRestriction *elem_restr_diff_flux, CeedBasis *basis_diff_flux) {
  ...
  PetscCall(PetscSectionSetComponentName(section, 0, 0, "DivergenceDiffusiveFlux_MomentumY"));
  PetscCall(PetscSectionSetComponentName(section, 0, 0, "DivergenceDiffusiveFlux_MomentumZ"));
  PetscCall(PetscSectionSetComponentName(section, 0, 0, "DivergenceDiffusiveFlux_Energy"));
...
PetscErrorCode DivDiffFluxProjectionSetup(Ceed ceed, User user, CeedData ceed_data, ProblemData *problem) {
  ...
  {  // Get elem_restr_diff_flux and basis_diff_flux
    ...
    PetscCallCeed(ceed, CeedOperatorGetFieldByName(user->op_ifunction_vol, "div F_diff", &op_field));
  ...
  switch (user->phys->state_var) {
    case STATEVAR_PRIMITIVE:
      PetscCallCeed(
          ceed, CeedQFunctionCreateInterior(ceed, 1, DivDiffusiveFluxRHS_Prim, DivDiffusiveFluxRHS_Prim_loc, &qf_rhs_assemble));
      break;
    case STATEVAR_CONSERVATIVE:
      PetscCallCeed(ceed, CeedQFunctionCreateInterior(ceed, 1, DivDiffusiveFluxRHS_Conserv, DivDiffusiveFluxRHS_Conserv_loc, 
                                                      &qf_rhs_assemble));
      break;
    default:
  ...
  PetscCallCeed(ceed, CeedQFunctionAddOutput(qf_rhs_assemble, "diffusive flux RHS", diff_flux_proj->num_comp, CEED_EVAL_INTERP));
  ...
  {  // -- Setup KSP for L^2 projection with lumped mass operator
    ...
    PetscCall(OperatorApplyContextCreate(diff_flux_proj->dm, diff_flux_proj->dm, ceed, op_mass, NULL, NULL, NULL, NULL, &mass_matop_ctx));
    ...
    PetscCall(KSPSetOptionsPrefix(diff_flux_proj->ksp, "div_diff_flux_projection_"));
    {
      ...
      PetscCall(KSPSetConvergenceTest(diff_flux_proj->ksp, KSPConvergedSkip, NULL, NULL));
    }
    PetscCall(KSPSetOperators(diff_flux_proj->ksp, mat_mass, mat_mass));
    PetscCall(KSPSetFromOptions(diff_flux_proj->ksp));
  }
