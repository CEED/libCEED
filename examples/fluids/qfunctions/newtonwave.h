...
CEED_QFUNCTION_HELPER int IC_NewtonianWave(void *ctx, CeedInt Q,
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    const CeedScalar perturbation = 1 + amplitude*exp(-(Square(x0) + Square(y0))/
                                    (2*Square(width)));

    const CeedScalar e_kinetic = 0.5*S_infty.U.density*Dot3(S_infty.Y.velocity,
                                 S_infty.Y.velocity);
...
CEED_QFUNCTION(IC_NewtonianWave_Conserv)(void *ctx, CeedInt Q,
    const CeedScalar *const *in, CeedScalar *const *out) {
  return IC_NewtonianWave(ctx, Q, in, out, StateToU);
}

CEED_QFUNCTION(IC_NewtonianWave_Prim)(void *ctx, CeedInt Q,
                                      const CeedScalar *const *in, CeedScalar *const *out) {
