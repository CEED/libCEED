...
// *****************************************************************************
// Helper function for computing flux Jacobian Prim
// *****************************************************************************
CEED_QFUNCTION_HELPER void computeFluxJacobian_NSp(CeedScalar dF[3][5][5],
    const CeedScalar rho, const CeedScalar u[3], const CeedScalar E,
    const CeedScalar Rd, const CeedScalar cv ) {
  CeedScalar u_sq = u[0]*u[0] + u[1]*u[1] + u[2]*u[2]; // Velocity square
  // someone else can add this in later but assume zero g below CeedScalar e_potential = -(g[0]*x[0] + g[1]*x[1] + g[2]*x[2]);
 ...
 //        [row][col] of A_i
      ...
      for (CeedInt k=0; k<3; k++) { // k counts the wrt vel_k this loop handles middle columns for all 5 rows
...
CEED_QFUNCTION_HELPER void PrimitiveToConservative_fwd(const CeedScalar rho, const CeedScalar u[3], const CeedScalar E,
    const CeedScalar Rd, const CeedScalar cv, const CeedScalar dY[5], CeedScalar dU[5]) {
