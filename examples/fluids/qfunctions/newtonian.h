...
//
// *****************************************************************************
CEED_QFUNCTION(IFunction_Newtonian)(void *ctx, CeedInt Q, const CeedScalar *const *in, CeedScalar *const *out) {
  // *INDENT-OFF*
  // Inputs
  const CeedScalar(*q)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[0], (*Grad_q)[5][CEED_Q_VLA] = (const CeedScalar(*)[5][CEED_Q_VLA])in[1],
  ...
  CeedPragmaSIMD
    ...
    const CeedScalar body_force[5] = {0, s.U.density * g[0], s.U.density * g[1], s.U.density * g[2], 0};
    for (CeedInt j = 0; j < 5; j++) v[j][i] = wdetJ * (q_dot[j][i] - body_force[j]);
...
//
// *****************************************************************************
CEED_QFUNCTION(IJacobian_Newtonian)(void *ctx, CeedInt Q, const CeedScalar *const *in, CeedScalar *const *out) {
  // *INDENT-OFF*
  // Inputs
  ...
  CeedPragmaSIMD
    ...
    for (int j = 0; j < 3; j++) {
      CeedScalar dUj[5];
      for (int k = 0; k < 5; k++) dUj[k] = Grad_dq[0][k][i] * dXdx[0][j] + Grad_dq[1][k][i] * dXdx[1][j] + Grad_dq[2][k][i] * dXdx[2][j];
...
// *****************************************************************************
CEED_QFUNCTION(BoundaryIntegral)(void *ctx, CeedInt Q, const CeedScalar *const *in, CeedScalar *const *out) {
  //*INDENT-OFF*
  const CeedScalar(*q)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[0], (*Grad_q)[5][CEED_Q_VLA] = (const CeedScalar(*)[5][CEED_Q_VLA])in[1],
  ...
  State (*StateFromQi)(NewtonianIdealGasContext gas, const CeedScalar qi[5], const CeedScalar x[3]);
  State (*StateFromQi_fwd)(NewtonianIdealGasContext gas, State s, const CeedScalar dqi[5], const CeedScalar x[3], const CeedScalar dx[3]);
  ...
  CeedPragmaSIMD for (CeedInt i = 0; i < Q; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      for (int k = 0; k < 3; k++) Flux[k + 1] += (F_inviscid[j].momentum[k] - stress[k][j]) * norm[j];
    ...
    // -- Momentum
    for (CeedInt j = 0; j < 3; j++) v[j + 1][i] = -wdetJb * Flux[j + 1];
...
// *****************************************************************************
CEED_QFUNCTION(BoundaryIntegral_Jacobian)(void *ctx, CeedInt Q, const CeedScalar *const *in, CeedScalar *const *out) {
  // *INDENT-OFF*
  // Inputs
  const CeedScalar(*dq)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[0], (*Grad_dq)[5][CEED_Q_VLA] = (const CeedScalar(*)[5][CEED_Q_VLA])in[1],
        (*q_data_sur)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[2], (*x)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[3],
  ...
  CeedPragmaSIMD
    ...
    for (int j = 0; j < 3; j++) {
      ...
      for (int k = 0; k < 3; k++) dFlux[k + 1] += (dF_inviscid[j].momentum[k] - dstress[k][j]) * norm[j];
...
// *****************************************************************************
CEED_QFUNCTION(PressureOutflow)(void *ctx, CeedInt Q, const CeedScalar *const *in, CeedScalar *const *out) {
  // *INDENT-OFF*
  // Inputs
  const CeedScalar(*q)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[0], (*Grad_q)[5][CEED_Q_VLA] = (const CeedScalar(*)[5][CEED_Q_VLA])in[1],
        (*q_data_sur)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[2], (*x)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[3];
...
// *****************************************************************************
CEED_QFUNCTION(PressureOutflow_Jacobian)(void *ctx, CeedInt Q, const CeedScalar *const *in, CeedScalar *const *out) {
  // *INDENT-OFF*
  // Inputs
  const CeedScalar(*dq)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[0], (*Grad_dq)[5][CEED_Q_VLA] = (const CeedScalar(*)[5][CEED_Q_VLA])in[1],
        (*q_data_sur)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[2], (*x)[CEED_Q_VLA] = (const CeedScalar(*)[CEED_Q_VLA])in[3],
...
CEED_QFUNCTION(IFunction_Newtonian_Prim)(void *ctx, CeedInt Q, const CeedScalar *const *in, CeedScalar *const *out) {
  ...
  CeedPragmaSIMD
    ...
    for (CeedInt j = 0; j < 3; j++) {
      ...
      for (CeedInt k = 0; k < 5; k++) dY[k] = Grad_q[0][k][i] * dXdx[0][j] + Grad_q[1][k][i] * dXdx[1][j] + Grad_q[2][k][i] * dXdx[2][j];
      dx_i[j]   = 1.;
...
CEED_QFUNCTION(IJacobian_Newtonian_Prim)(void *ctx, CeedInt Q, const CeedScalar *const *in, CeedScalar *const *out) {
  ...
  CeedPragmaSIMD
    ...
    for (int j = 0; j < 3; j++) {
      CeedScalar dYj[5];
      for (int k = 0; k < 5; k++) dYj[k] = Grad_dq[0][k][i] * dXdx[0][j] + Grad_dq[1][k][i] * dXdx[1][j] + Grad_dq[2][k][i] * dXdx[2][j];
