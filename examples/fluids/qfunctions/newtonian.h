...
//
// *****************************************************************************
CEED_QFUNCTION_HELPER int IFunction_Newtonian_Generic(void *ctx, CeedInt Q,
    const CeedScalar *const *in, CeedScalar *const *out,
    State (*pStateFromQi)(NewtonianIdealGasContext gas,
                          const CeedScalar qi[5], const CeedScalar x[3]),
    State (*pStateFromQi_fwd)(NewtonianIdealGasContext gas,
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    const CeedScalar x_i[3] = {x[0][i], x[1][i], x[2][i]};
    State s = pStateFromQi(context, qi, x_i);
    ...
    for (CeedInt j=0; j<3; j++) {
      ...
      for (CeedInt k=0; k<5; k++)
        dqi[k] = Grad_q[0][k][i] * dXdx[0][j] +
                 ...
                 Grad_q[2][k][i] * dXdx[2][j];
      dx_i[j] = 1.;
      grad_s[j] = pStateFromQi_fwd(context, s, dqi, x_i, dx_i);
    }
    ...
    if (context->use_primitive) {
      CeedScalar qi_dot[5], dx0[3] = {0};
      for (int j=0; j<5; j++) qi_dot[j] = q_dot[j][i];
      State s_dot = StateFromY_fwd(context, s, qi_dot, x_i, dx0);
...
CEED_QFUNCTION_HELPER int IJacobian_Newtonian_Generic(void *ctx, CeedInt Q,
    const CeedScalar *const *in, CeedScalar *const *out,
    State (*pStateFromQi)(NewtonianIdealGasContext gas,
                          const CeedScalar qi[5], const CeedScalar x[3]),
    State (*pStateFromQi_fwd)(NewtonianIdealGasContext gas,
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    State ds = pStateFromQi_fwd(context, s, dqi, x_i, dx0);
    ...
    for (int j=0; j<3; j++) {
      ...
      grad_ds[j] = pStateFromQi_fwd(context, s, dqi_j, x_i, dx0);
    }
...
CEED_QFUNCTION(IFunction_Newtonian)(void *ctx, CeedInt Q,
                                    const CeedScalar *const *in, CeedScalar *const *out) {
  return IFunction_Newtonian_Generic(ctx, Q, in, out, StateFromU,
                                     StateFromU_fwd);
}
...
CEED_QFUNCTION(IJacobian_Newtonian)(void *ctx, CeedInt Q,
                                    const CeedScalar *const *in, CeedScalar *const *out) {
  return IJacobian_Newtonian_Generic(ctx, Q, in, out, StateFromU,
                                     StateFromU_fwd);
}
...
CEED_QFUNCTION(IFunction_Newtonian_Prim)(void *ctx, CeedInt Q,
    const CeedScalar *const *in, CeedScalar *const *out) {
  return IFunction_Newtonian_Generic(ctx, Q, in, out, StateFromY,
                                     StateFromY_fwd);
}
...
CEED_QFUNCTION(IJacobian_Newtonian_Prim)(void *ctx, CeedInt Q,
    const CeedScalar *const *in, CeedScalar *const *out) {
  return IJacobian_Newtonian_Generic(ctx, Q, in, out, StateFromY,
                                     StateFromY_fwd);
}
