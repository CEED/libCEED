...
CEED_QFUNCTION_HELPER void SwapInt(CeedInt *a, CeedInt *b) {
  CeedInt temp = *a;
  *a           = *b;
  *b           = temp;
}
...
CEED_QFUNCTION_HELPER void CopyN(const CeedScalar *x, CeedScalar *y, const CeedInt N) {
  CeedPragmaSIMD for (CeedInt i = 0; i < N; i++) { y[i] = x[i]; }
}
...
CEED_QFUNCTION_HELPER void MatVecNM(const CeedScalar *A, const CeedScalar *x, const CeedInt N, const CeedInt M, const UtilMatrixTranspose transpose_A,
                                    CeedScalar *b) {
  switch (transpose_A) {
    case NO_TRANSPOSE:
      ...
      break;
    case TRANSPOSE:
      CeedPragmaSIMD for (CeedInt i = 0; i < M; i++) {
        CeedPragmaSIMD for (CeedInt j = 0; j < N; j++) { b[i] += A[j * M + i] * x[j]; }
      }
      break;
  }
}
...
CEED_QFUNCTION_HELPER void MatVec3(const CeedScalar A[3][3], const CeedScalar x[3], const UtilMatrixTranspose transpose_A, CeedScalar b[3]) {
  MatVecNM((const CeedScalar *)A, (const CeedScalar *)x, 3, 3, transpose_A, (CeedScalar *)b);
}

// @brief Matrix vector product with pointwise scaling of output.
// @details b = DAx + b. A is NxM, x is M, b is N, D is diagonal NxN, represented by a vector of length N
CEED_QFUNCTION_HELPER void MatVecDiagNM(const CeedScalar *A, const CeedScalar *x, const CeedScalar *D, const CeedInt N, const CeedInt M,
                                        const UtilMatrixTranspose transpose_A, CeedScalar *b) {
  switch (transpose_A) {
    case NO_TRANSPOSE:
      CeedPragmaSIMD for (CeedInt i = 0; i < N; i++) { b[i] += D[i] * DotN(&A[i * M], x, M); }
      break;
    case TRANSPOSE:
      CeedPragmaSIMD for (CeedInt i = 0; i < M; i++) {
        CeedPragmaSIMD for (CeedInt j = 0; j < N; j++) { b[i] += D[i] * A[j * M + i] * x[j]; }
      }
      break;
  }
}

// @brief 3x3 Matrix vector product with pointwise scaling of output. b = DAx + b.
// @details D is diagonal, represented by a vector of length 3
CEED_QFUNCTION_HELPER void MatVecDiag3(const CeedScalar A[3][3], const CeedScalar x[3], const CeedScalar D[3], const UtilMatrixTranspose transpose_A,
                                       CeedScalar b[3]) {
  MatVecDiagNM((const CeedScalar *)A, (const CeedScalar *)x, (const CeedScalar *)D, 3, 3, transpose_A, (CeedScalar *)b);
}
...
CEED_QFUNCTION_HELPER void MatDiagNM(const CeedScalar *A, const CeedScalar *D, const CeedInt N, const CeedInt M,
                                     const UtilMatrixTranspose transpose_A, CeedScalar *B) {
  switch (transpose_A) {
    case NO_TRANSPOSE:
      CeedPragmaSIMD for (CeedInt i = 0; i < N; i++) {
        CeedPragmaSIMD for (CeedInt j = 0; j < M; j++) { B[i * M + j] += D[i] * A[i * M + j]; }
      }
      break;
    case TRANSPOSE:
      CeedPragmaSIMD for (CeedInt i = 0; i < M; i++) {
        CeedPragmaSIMD for (CeedInt j = 0; j < N; j++) { B[i * N + j] += D[i] * A[j * M + i]; }
      }
      break;
  }
}
...
CEED_QFUNCTION_HELPER void MatDiag3(const CeedScalar A[3][3], const CeedScalar D[3], const UtilMatrixTranspose transpose_A, CeedScalar B[3][3]) {
  MatDiagNM((const CeedScalar *)A, (const CeedScalar *)D, 3, 3, transpose_A, (CeedScalar *)B);
}

// @brief Matrix-Matrix product, B = AC + B
// @details A is NxM, C is MxO
CEED_QFUNCTION_HELPER void MatMatNMO(const CeedScalar *A, const CeedScalar *B, const CeedInt N, const CeedInt M, const CeedInt O, CeedScalar *C) {
  CeedPragmaSIMD for (CeedInt i = 0; i < N; i++) {
    CeedPragmaSIMD for (CeedInt j = 0; j < O; j++) {
      CeedPragmaSIMD for (CeedInt k = 0; k < M; k++) { C[i * O + j] += A[i * M + k] * B[k * O + j]; }
    }
  }
}

// @brief 3x3 Matrix-Matrix product, C = AB + C
CEED_QFUNCTION_HELPER void MatMat3(const CeedScalar A[3][3], const CeedScalar B[3][3], const UtilMatrixTranspose transpose_A,
                                   const UtilMatrixTranspose transpose_B, CeedScalar C[3][3]) {
  switch (transpose_A) {
    case NO_TRANSPOSE:
      switch (transpose_B) {
        case NO_TRANSPOSE:
          CeedPragmaSIMD for (CeedInt i = 0; i < 3; i++) {
            CeedPragmaSIMD for (CeedInt j = 0; j < 3; j++) {
              CeedPragmaSIMD for (CeedInt k = 0; k < 3; k++) { C[i][j] += A[i][k] * B[k][j]; }
            }
          }
          break;
        case TRANSPOSE:
          CeedPragmaSIMD for (CeedInt i = 0; i < 3; i++) {
            CeedPragmaSIMD for (CeedInt j = 0; j < 3; j++) {
              CeedPragmaSIMD for (CeedInt k = 0; k < 3; k++) { C[i][j] += A[i][k] * B[j][k]; }
            }
          }
          break;
      }
    ...
    case TRANSPOSE:
      switch (transpose_B) {
        case NO_TRANSPOSE:
          CeedPragmaSIMD for (CeedInt i = 0; i < 3; i++) {
            CeedPragmaSIMD for (CeedInt j = 0; j < 3; j++) {
              CeedPragmaSIMD for (CeedInt k = 0; k < 3; k++) { C[i][j] += A[k][i] * B[k][j]; }
            }
          }
          break;
        case TRANSPOSE:
          CeedPragmaSIMD for (CeedInt i = 0; i < 3; i++) {
            CeedPragmaSIMD for (CeedInt j = 0; j < 3; j++) {
              CeedPragmaSIMD for (CeedInt k = 0; k < 3; k++) { C[i][j] += A[k][i] * B[j][k]; }
            }
          }
          break;
      }
...
CEED_QFUNCTION_HELPER void KMMetricTensor(const CeedScalar dXdx[3][3], CeedScalar km_g_ij[6]) {
  ...
  MatMat3(dXdx, dXdx, TRANSPOSE, NO_TRANSPOSE, g_ij);
  KMPack(g_ij, km_g_ij);
}
