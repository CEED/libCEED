...
// *****************************************************************************
static CEED_QFUNCTION_HELPER int Exact_Advection2d(CeedInt dim, CeedScalar time,
...
CEED_QFUNCTION(IFunction_Advection2d)(void *ctx, CeedInt Q,
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    for (CeedInt j=0; j<2; j++)
      ...
      case 0:
        break;
      case 1: dv[j][4][i] += wdetJ * TauS * strong_conv * uX[j];
        break;
      case 2: dv[j][4][i] += wdetJ * TauS * strong_res * uX[j];
        break;
      }
  } // End Quadrature Point Loop
...
CEED_QFUNCTION(Advection2d_Sur)(void *ctx, CeedInt Q,
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    // ---- Normal vectors
    const CeedScalar norm[2]    =   {q_data_sur[1][i],
                                     q_data_sur[2][i]
                                    };
    // Normal velocity
