...
CEED_QFUNCTION(SetupMMSForce)(void *ctx, const CeedInt Q,
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    // Forcing function
    // -- Component 1
    force[i+0*Q] = wJ*(-(E*(cos(x*2.0)*cos(y*3.0)*exp(z*4.0)*4.0 -
                       ...
                       ((nu*2.0-1.0)*(nu+1.0))) / 1e8;

    // -- Component 2
    force[i+1*Q] = wJ*(-(E*(cos(y*3.0)*cos(z*4.0)*exp(x*2.0)*3.0 -
    ...
    // -- Component 3
    force[i+2*Q] = wJ*(-(E*(cos(x*2.0)*cos(z*4.0)*exp(y*3.0)*6.0 -
