...
The `/cpu/self/avx/*`, `/cpu/self/sve/*`, `/cpu/self/sme/*`, and `/cpu/self/xsmm/*` backends delegate to the corresponding `/cpu/self/opt/*` backends.
These backends update the `CeedTensorContract` objects using AVX intrinsics, vector-length-agnostic ARM SVE intrinsics, vector-length-agnostic ARM SME outer-product intrinsics, and libXSMM functions, respectively.
