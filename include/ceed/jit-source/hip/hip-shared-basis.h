...
inline __device__ void loadMatrix(const CeedScalar* d_B, CeedScalar* B) {
  ...
  for (CeedInt i = tid; i < BASIS_P_1D*BASIS_Q_1D; i += blockDim.x*blockDim.y*blockDim.z)
    B[i] = d_B[i];
}
...
// Read DoFs
//------------------------------------------------------------------------------
inline __device__ void readDofs1d(const CeedInt elem, const CeedInt t_id_x,
                                  const CeedInt t_id_y, const CeedInt t_id_z,const CeedInt comp,
                                  const CeedInt num_elem, const CeedScalar *d_U,
                                  CeedScalar *slice) {
  for (CeedInt i = 0; i < BASIS_P_1D; i++)
    slice[i + t_id_z*BASIS_T_1D] = d_U[i + elem*BASIS_P_1D + comp*BASIS_P_1D*num_elem];
  for (CeedInt i = BASIS_P_1D; i < BASIS_Q_1D; i++)
    slice[i + t_id_z*BASIS_T_1D] = 0.0;
}

//------------------------------------------------------------------------------
// Write DoFs
//------------------------------------------------------------------------------
inline __device__ void writeDofs1d(const CeedInt elem, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt comp,
                                   const CeedInt num_elem, const CeedScalar &r_V,
                                   CeedScalar *d_V) {
  if (t_id_x < BASIS_P_1D)
    d_V[t_id_x + elem*BASIS_P_1D + comp*BASIS_P_1D*num_elem] = r_V;
}

//------------------------------------------------------------------------------
// Read quadrature point data
//------------------------------------------------------------------------------
inline __device__ void readQuads1d(const CeedInt elem, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt t_id_z, const CeedInt comp,
                                   const CeedInt dim, const CeedInt num_elem,
                                   const CeedScalar *d_U, CeedScalar *slice) {
  for (CeedInt i = 0; i < BASIS_Q_1D; i++)
    slice[i + t_id_z*BASIS_T_1D] = d_U[i + elem*BASIS_Q_1D + comp*BASIS_Q_1D*num_elem +
...
//------------------------------------------------------------------------------
inline __device__ void writeQuads1d(const CeedInt elem, const CeedInt t_id_x,
                                    const CeedInt t_id_y, const CeedInt comp,
                                    const CeedInt dim, const CeedInt num_elem,
                                    const CeedScalar &r_V, CeedScalar *d_V) {
  if (t_id_x<BASIS_Q_1D)
    d_V[t_id_x + elem*BASIS_Q_1D + comp*BASIS_Q_1D*num_elem + dim*BASIS_NUM_COMP*num_elem*BASIS_Q_1D] = r_V;
}

//------------------------------------------------------------------------------
// 1D tensor contraction
//------------------------------------------------------------------------------
inline __device__ void ContractX1d(CeedScalar *slice, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt t_id_z,
                                   const CeedScalar &U, const CeedScalar *B,
                                   CeedScalar &V) {
  V = 0.0;
  for (CeedInt i = 0; i < BASIS_P_1D; i++)
    V += B[i + t_id_x*BASIS_P_1D] * slice[i + t_id_z*BASIS_T_1D]; // Contract x direction
}

//------------------------------------------------------------------------------
// 1D transpose tensor contraction
//------------------------------------------------------------------------------
inline __device__ void ContractTransposeX1d(CeedScalar *slice, const CeedInt t_id_x,
    const CeedInt t_id_y, const CeedInt t_id_z,
    const CeedScalar &U, const CeedScalar *B, CeedScalar &V) {
  V = 0.0;
  for (CeedInt i = 0; i < BASIS_Q_1D; i++)
    V += B[t_id_x + i*BASIS_P_1D] * slice[i + t_id_z*BASIS_T_1D]; // Contract x direction
}
...
// Read DoFs
//------------------------------------------------------------------------------
inline __device__ void readDofs2d(const CeedInt elem, const CeedInt t_id_x,
                                  const CeedInt t_id_y, const CeedInt comp,
                                  const CeedInt num_elem, const CeedScalar *d_U,
                                  CeedScalar &U) {
  U = (t_id_x<BASIS_P_1D && t_id_y<BASIS_P_1D) ?
      d_U[t_id_x + t_id_y*BASIS_P_1D + elem*BASIS_P_1D*BASIS_P_1D + comp*BASIS_P_1D*BASIS_P_1D*num_elem] : 0.0;
}

//------------------------------------------------------------------------------
// Write DoFs
//------------------------------------------------------------------------------
inline __device__ void writeDofs2d(const CeedInt elem, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt comp,
                                   const CeedInt num_elem, const CeedScalar &r_V,
                                   CeedScalar *d_V) {
...
// Read quadrature point data
//------------------------------------------------------------------------------
inline __device__ void readQuads2d(const CeedInt elem, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt comp,
                                   const CeedInt dim, const CeedInt num_elem,
  ...
  U = (t_id_x<BASIS_Q_1D && t_id_y<BASIS_Q_1D) ?
      ...
      dim*BASIS_NUM_COMP*num_elem*BASIS_Q_1D*BASIS_Q_1D] : 0.0;
}

//------------------------------------------------------------------------------
// Write quadrature point data
//------------------------------------------------------------------------------
inline __device__ void writeQuads2d(const CeedInt elem, const CeedInt t_id_x,
                                    const CeedInt t_id_y, const CeedInt comp,
                                    const CeedInt dim, const CeedInt num_elem,
                                    const CeedScalar &r_V, CeedScalar *d_V) {
...
//------------------------------------------------------------------------------
inline __device__ void ContractX2d(CeedScalar *slice, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt t_id_z,
                                   const CeedScalar &U, const CeedScalar *B,
                                   CeedScalar &V) {
...
//------------------------------------------------------------------------------
inline __device__ void ContractY2d(CeedScalar *slice, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt t_id_z,
                                   const CeedScalar &U, const CeedScalar *B,
                                   CeedScalar &V) {
  slice[t_id_x + t_id_y*BASIS_T_1D + t_id_z*BASIS_T_1D*BASIS_T_1D] = U;
...
//------------------------------------------------------------------------------
inline __device__ void ContractTransposeY2d(CeedScalar *slice, const CeedInt t_id_x,
    const CeedInt t_id_y, const CeedInt t_id_z,
    const CeedScalar &U, const CeedScalar *B, CeedScalar &V) {
  slice[t_id_x + t_id_y*BASIS_T_1D + t_id_z*BASIS_T_1D*BASIS_T_1D] = U;
  __syncthreads();
...
//------------------------------------------------------------------------------
inline __device__ void ContractTransposeX2d(CeedScalar *slice, const CeedInt t_id_x,
    const CeedInt t_id_y, const CeedInt t_id_z,
    const CeedScalar &U, const CeedScalar *B, CeedScalar &V) {
  slice[t_id_x + t_id_y*BASIS_T_1D + t_id_z*BASIS_T_1D*BASIS_T_1D] = U;
  __syncthreads();
...
// Read DoFs
//------------------------------------------------------------------------------
inline __device__ void readDofs3d(const CeedInt elem, const CeedInt t_id_x,
                                  const CeedInt t_id_y, const CeedInt comp,
                                  const CeedInt num_elem, const CeedScalar *d_U,
                                  CeedScalar *r_U) {
  for (CeedInt i = 0; i < BASIS_P_1D; i++)
...
// Write DoFs
//------------------------------------------------------------------------------
inline __device__ void writeDofs3d(const CeedInt elem, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt comp,
                                   const CeedInt num_elem, const CeedScalar *r_V,
                                   CeedScalar *d_V) {
  if (t_id_x < BASIS_P_1D && t_id_y < BASIS_P_1D) {
...
// Read quadrature point data
//------------------------------------------------------------------------------
inline __device__ void readQuads3d(const CeedInt elem, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt comp,
                                   const CeedInt dim, const CeedInt num_elem,
                                   const CeedScalar *d_U, CeedScalar *r_U) {
  for (CeedInt i = 0; i < BASIS_Q_1D; i++)
    r_U[i] = (t_id_x < BASIS_Q_1D && t_id_y < BASIS_Q_1D) ? 
              d_U[t_id_x + t_id_y*BASIS_Q_1D + i*BASIS_Q_1D*BASIS_Q_1D + elem*BASIS_Q_1D*BASIS_Q_1D*BASIS_Q_1D +
              comp*BASIS_Q_1D*BASIS_Q_1D*BASIS_Q_1D*num_elem + dim*BASIS_NUM_COMP*num_elem*BASIS_Q_1D*BASIS_Q_1D*BASIS_Q_1D] : 0.0;
  for (CeedInt i = BASIS_Q_1D; i < BASIS_P_1D; i++)
    r_U[i] = 0.0;
}
...
// Write quadrature point data
//------------------------------------------------------------------------------
inline __device__ void writeQuads3d(const CeedInt elem, const CeedInt t_id_x,
                                    const CeedInt t_id_y, const CeedInt comp,
                                    const CeedInt dim, const CeedInt num_elem,
                                    const CeedScalar *r_V, CeedScalar *d_V) {
  if (t_id_x < BASIS_Q_1D && t_id_y < BASIS_Q_1D) {
    for (CeedInt i = 0; i < BASIS_Q_1D; i++)
      d_V[t_id_x + t_id_y*BASIS_Q_1D + i*BASIS_Q_1D*BASIS_Q_1D + elem*BASIS_Q_1D*BASIS_Q_1D*BASIS_Q_1D + comp*BASIS_Q_1D*BASIS_Q_1D*BASIS_Q_1D*num_elem +
          dim*BASIS_NUM_COMP*num_elem*BASIS_Q_1D*BASIS_Q_1D*BASIS_Q_1D] = r_V[i];
  }
}
...
//------------------------------------------------------------------------------
inline __device__ void ContractX3d(CeedScalar *slice, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt t_id_z,
                                   const CeedScalar *U,
                                   const CeedScalar *B,
                                   CeedScalar *V) {
...
//------------------------------------------------------------------------------
inline __device__ void ContractY3d(CeedScalar *slice, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt t_id_z,
                                   const CeedScalar *U,
                                   const CeedScalar *B,
                                   CeedScalar *V) {
...
//------------------------------------------------------------------------------
inline __device__ void ContractZ3d(CeedScalar *slice, const CeedInt t_id_x,
                                   const CeedInt t_id_y, const CeedInt t_id_z,
                                   const CeedScalar *U,
                                   const CeedScalar *B,
                                   CeedScalar *V) {
...
//------------------------------------------------------------------------------
inline __device__ void ContractTransposeZ3d(CeedScalar *slice, const CeedInt t_id_x,
                                            const CeedInt t_id_y, const CeedInt t_id_z,
                                            const CeedScalar *U,
                                            const CeedScalar *B,
                                            CeedScalar *V) {
...
//------------------------------------------------------------------------------
inline __device__ void ContractTransposeY3d(CeedScalar *slice, const CeedInt t_id_x,
                                            const CeedInt t_id_y, const CeedInt t_id_z,
                                            const CeedScalar *U,
                                            const CeedScalar *B,
                                            CeedScalar *V) {
...
//------------------------------------------------------------------------------
inline __device__ void ContractTransposeX3d(CeedScalar *slice, const CeedInt t_id_x,
                                            const CeedInt t_id_y, const CeedInt t_id_z,
                                            const CeedScalar *U,
                                            const CeedScalar *B,
                                            CeedScalar *V) {
...
//------------------------------------------------------------------------------
extern "C" __launch_bounds__(BASIS_INTERP_BLOCK_SIZE)
           __global__ void Interp(const CeedInt num_elem, const CeedInt transpose,
...
// Grad kernel by dim
//------------------------------------------------------------------------------
extern "C" __launch_bounds__(BASIS_GRAD_BLOCK_SIZE)
           __global__ void Grad(const CeedInt num_elem, const CeedInt transpose,
...
//------------------------------------------------------------------------------
extern "C" __launch_bounds__(BASIS_WEIGHT_BLOCK_SIZE)
