...
static inline int CeedBasis_Apply_Transpose_NonTensor(const CeedScalar *mat, const CeedScalar *u, CeedScalar *v) {
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP, Q, BLOCK_SIZE, P>(mat, u, v));
...
static inline int CeedBasis_Apply_NoTranspose_Interp_Tensor_1D(const CeedScalar *interp_1d, const CeedScalar *u, CeedScalar *v) {
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, P_1D, BLOCK_SIZE, Q_1D>(interp_1d, u, v));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_NoTranspose_Interp_Tensor_2D(const CeedScalar *interp_1d, const CeedScalar *u, CeedScalar *v) {
  ...
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D, P_1D, BLOCK_SIZE, Q_1D>(interp_1d, u, temp));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, P_1D, BLOCK_SIZE * Q_1D, Q_1D>(interp_1d, temp, v));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_NoTranspose_Interp_Tensor_3D(const CeedScalar *interp_1d, const CeedScalar *u, CeedScalar *v) {
  ...
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D * P_1D, P_1D, BLOCK_SIZE, Q_1D>(interp_1d, u, temp_0));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D, P_1D, BLOCK_SIZE * Q_1D, Q_1D>(interp_1d, temp_0, temp_1));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, P_1D, BLOCK_SIZE * Q_1D * Q_1D, Q_1D>(interp_1d, temp_1, v));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_Transpose_Interp_Tensor_1D(const CeedScalar *interp_1d, const CeedScalar *u, CeedScalar *v) {
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP, Q_1D, BLOCK_SIZE, P_1D>(interp_1d, u, v));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_Transpose_Interp_Tensor_2D(const CeedScalar *interp_1d, const CeedScalar *u, CeedScalar *v) {
  ...
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D, Q_1D, BLOCK_SIZE, P_1D>(interp_1d, u, temp));
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP, Q_1D, BLOCK_SIZE * P_1D, P_1D>(interp_1d, temp, v));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_Transpose_Interp_Tensor_3D(const CeedScalar *interp_1d, const CeedScalar *u, CeedScalar *v) {
  ...
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D * Q_1D, Q_1D, BLOCK_SIZE, P_1D>(interp_1d, u, temp_0));
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D, Q_1D, BLOCK_SIZE * P_1D, P_1D>(interp_1d, temp_0, temp_1));
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP, Q_1D, BLOCK_SIZE * P_1D * P_1D, P_1D>(interp_1d, temp_1, v));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_NoTranspose_Grad_Tensor_1D(const CeedScalar *interp_1d, const CeedScalar *grad_1d, const CeedScalar *u,
                                                             CeedScalar *v) {
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, P_1D, BLOCK_SIZE, Q_1D>(grad_1d, u, v));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_NoTranspose_Grad_Tensor_2D(const CeedScalar *interp_1d, const CeedScalar *grad_1d, const CeedScalar *u,
  ...
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D, P_1D, BLOCK_SIZE, Q_1D>(interp_1d, u, temp));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, P_1D, BLOCK_SIZE * Q_1D, Q_1D>(grad_1d, temp, v));

  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D, P_1D, BLOCK_SIZE, Q_1D>(grad_1d, u, temp));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, P_1D, BLOCK_SIZE * Q_1D, Q_1D>(interp_1d, temp, &v[BLOCK_SIZE * NUM_COMP * Q_1D * Q_1D]));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_NoTranspose_Grad_Tensor_3D(const CeedScalar *interp_1d, const CeedScalar *grad_1d, const CeedScalar *u,
  ...
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D * P_1D, P_1D, BLOCK_SIZE, Q_1D>(interp_1d, u, temp_0));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D, P_1D, BLOCK_SIZE * Q_1D, Q_1D>(interp_1d, temp_0, temp_1));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, P_1D, BLOCK_SIZE * Q_1D * Q_1D, Q_1D>(grad_1d, temp_1, v));

  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D * P_1D, P_1D, BLOCK_SIZE, Q_1D>(interp_1d, u, temp_0));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D, P_1D, BLOCK_SIZE * Q_1D, Q_1D>(grad_1d, temp_0, temp_1));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, P_1D, BLOCK_SIZE * Q_1D * Q_1D, Q_1D>(interp_1d, temp_1,
                                                                                            &v[1 * BLOCK_SIZE * NUM_COMP * Q_1D * Q_1D * Q_1D]));

  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D * P_1D, P_1D, BLOCK_SIZE, Q_1D>(interp_1d, u, temp_0));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D, P_1D, BLOCK_SIZE * Q_1D, Q_1D>(interp_1d, temp_0, temp_1));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, P_1D, BLOCK_SIZE * Q_1D * Q_1D, Q_1D>(grad_1d, temp_1,
...
static inline int CeedBasis_Apply_NoTranspose_Grad_Collo_Tensor_3D(const CeedScalar *interp_1d, const CeedScalar *collo_grad_1d, const CeedScalar *u,
  ...
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D * P_1D, P_1D, BLOCK_SIZE, Q_1D>(interp_1d, u, temp_0));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * P_1D, P_1D, BLOCK_SIZE * Q_1D, Q_1D>(interp_1d, temp_0, temp_1));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, P_1D, BLOCK_SIZE * Q_1D * Q_1D, Q_1D>(interp_1d, temp_1, temp_0));

  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * Q_1D * Q_1D, Q_1D, BLOCK_SIZE, Q_1D>(collo_grad_1d, temp_0, v));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP * Q_1D, Q_1D, BLOCK_SIZE * Q_1D, Q_1D>(collo_grad_1d, temp_0,
                                                                                            &v[1 * BLOCK_SIZE * NUM_COMP * Q_1D * Q_1D * Q_1D]));
  CeedCall(TensorContract_Apply_NoTranspose<NUM_COMP, Q_1D, BLOCK_SIZE * Q_1D * Q_1D, Q_1D>(collo_grad_1d, temp_0,
                                                                                            &v[2 * BLOCK_SIZE * NUM_COMP * Q_1D * Q_1D * Q_1D]));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_Transpose_Grad_Tensor_1D(const CeedScalar *interp_1d, const CeedScalar *grad_1d, const CeedScalar *u,
                                                           CeedScalar *v) {
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP, Q_1D, BLOCK_SIZE, P_1D>(grad_1d, u, v));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_Transpose_Grad_Tensor_2D(const CeedScalar *interp_1d, const CeedScalar *grad_1d, const CeedScalar *u,
  ...
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D, Q_1D, BLOCK_SIZE, P_1D>(grad_1d, u, temp));
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP, P_1D, BLOCK_SIZE * P_1D, P_1D>(interp_1d, temp, v));

  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D, Q_1D, BLOCK_SIZE, P_1D>(interp_1d, u[BLOCK_SIZE * NUM_COMP * Q_1D * Q_1D], temp));
  CeedCall(TensorContract_ApplyAdd_Transpose<NUM_COMP, Q_1D, BLOCK_SIZE * P_1D, P_1D>(grad_1d, temp, v));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_Transpose_Grad_Tensor_3D(const CeedScalar *interp_1d, const CeedScalar *grad_1d, const CeedScalar *u,
  ...
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D * Q_1D, Q_1D, BLOCK_SIZE, P_1D>(grad_1d, u, temp_0));
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D, Q_1D, BLOCK_SIZE * P_1D, P_1D>(interp_1d, temp_0, temp_1));
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP, Q_1D, BLOCK_SIZE * P_1D * P_1D, P_1D>(interp_1d, temp_1, v));

  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D * Q_1D, Q_1D, BLOCK_SIZE, P_1D>(interp_1d,
                                                                                          &u[1 * BLOCK_SIZE * NUM_COMP * Q_1D * Q_1D * Q_1D],
                                                                                          temp_0));
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D, Q_1D, BLOCK_SIZE * P_1D, P_1D>(grad_1d, temp_0, temp_1));
  CeedCall(TensorContract_ApplyAdd_Transpose<NUM_COMP, Q_1D, BLOCK_SIZE * P_1D * P_1D, P_1D>(interp_1d, temp_1, v));

  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D * Q_1D, Q_1D, BLOCK_SIZE, P_1D>(interp_1d,
                                                                                          &u[2 * BLOCK_SIZE * NUM_COMP * Q_1D * Q_1D * Q_1D],
                                                                                          temp_0));
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D, Q_1D, BLOCK_SIZE * P_1D, P_1D>(interp_1d, temp_0, temp_1));
  CeedCall(TensorContract_ApplyAdd_Transpose<NUM_COMP, Q_1D, BLOCK_SIZE * P_1D * P_1D, P_1D>(grad_1d, temp_1, v));
  return CEED_ERROR_SUCCESS;
}
...
static inline int CeedBasis_Apply_Transpose_Grad_Collo_Tensor_3D(const CeedScalar *interp_1d, const CeedScalar *collo_grad_1d, const CeedScalar *u,
  ...
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D * Q_1D, Q_1D, BLOCK_SIZE, Q_1D>(collo_grad_1d, u, temp_0));
  CeedCall(TensorContract_ApplyAdd_Transpose<NUM_COMP * Q_1D, Q_1D, BLOCK_SIZE * Q_1D, Q_1D>(collo_grad_1d,
                                                                                             &u[1 * BLOCK_SIZE * NUM_COMP * Q_1D * Q_1D * Q_1D],
                                                                                             temp_0));
  CeedCall(TensorContract_ApplyAdd_Transpose<NUM_COMP, Q_1D, BLOCK_SIZE * Q_1D * Q_1D, Q_1D>(collo_grad_1d,
                                                                                             &u[2 * BLOCK_SIZE * NUM_COMP * Q_1D * Q_1D * Q_1D],
                                                                                             temp_0));

  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D * Q_1D, Q_1D, BLOCK_SIZE, P_1D>(interp_1d, temp_0, temp_1));
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP * Q_1D, Q_1D, BLOCK_SIZE * P_1D, P_1D>(interp_1d, temp_1, temp_0));
  CeedCall(TensorContract_Apply_Transpose<NUM_COMP, Q_1D, BLOCK_SIZE * P_1D * P_1D, P_1D>(interp_1d, temp_0, v));
  return CEED_ERROR_SUCCESS;
}
