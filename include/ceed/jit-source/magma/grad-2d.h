...
//////////////////////////////////////////////////////////////////////////////////////////
extern "C" __launch_bounds__(MAGMA_BASIS_BOUNDS(MAXPQ, MAGMA_MAXTHREADS_2D)) __global__ void
          ...
          CeedScalar *dV, const int estrdV, const int cstrdV, const int dstrdV, const int nelem)
{
    ...
    readV_2d<CeedScalar, P, 1, NCOMP, P, 0>
    (dV + (0*dstrdV), cstrdV, rV, tx);

    /* read U (idim = 0 for dU, iDIM = 0 for rU) -- 
       there is a sync at the end of this function */
    readU_2d<CeedScalar, Q, 1, NCOMP, Q, 0>
    (dU + (0 * dstrdU), cstrdU, rU, sTmp, tx);
    /* first call (iDIM = 0, iDIMU = 0, iDIMV = 0) */
    magma_grad_2d_device<CeedScalar, 1, 1, NCOMP, Q, P, Q, P, 0, 0, 0>
    (sTinterp, sTgrad, rU, rV, beta, tx, rTmp, sTmp);
    /* there is a sync at the end of magma_grad_2d_device */
    ...
    /* second call (iDIM = 1, iDIMU = 0, iDIMV = 0) */
    magma_grad_2d_device<CeedScalar, 1, 1, NCOMP, Q, P, Q, P, 1, 0, 0>
    (sTinterp, sTgrad, rU, rV, beta, tx, rTmp, sTmp);
    /* there is a sync at the end of magma_grad_2d_device */

    // write V
    writeV_2d<CeedScalar, P, 1, NCOMP, P, 0>
    (dV + (0*dstrdV), cstrdV, rV, tx);
}
