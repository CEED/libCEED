...
magma_grad_3d_device( 
    ...
    T rU[DIM_U][NCOMP_][rUsize] , T rV[DIM_V][NCOMP_][rVsize], 
...
//////////////////////////////////////////////////////////////////////////////////////////
extern "C" __launch_bounds__(MAGMA_BASIS_BOUNDS(MAXPQ*MAXPQ, MAGMA_MAXTHREADS_3D)) __global__ void
magma_gradn_3d_kernel(
    const CeedScalar* dinterp1d, const CeedScalar* dgrad1d,
    const CeedScalar *dU, const int estrdU, const int cstrdU, const int dstrdU,  
          CeedScalar *dV, const int estrdV, const int cstrdV, const int dstrdV, const int nelem)
{
    ...
    magma_grad_3d_device<CeedScalar, 1, 1, NCOMP, P, Q, P, Q, 2, 0, 0>
    (sTinterp, sTgrad, rU, rV, beta, tx, rTmp, sTmp);
    /* there is a sync at the end of magma_grad_3d_device */
    writeV_3d<CeedScalar, Q, 1, NCOMP, Q, 0>
          ...
          CeedScalar *dV, const int estrdV, const int cstrdV, const int dstrdV, const int nelem)
{
    ...
    readV_3d<CeedScalar, P, 1, NCOMP, P, 0>
    (dV + (0*dstrdV), cstrdV, rV, tx);

    /* read U (idim = 0 for dU, iDIM = 0 for rU) -- 
       there is a sync at the end of this function */
    readU_3d<CeedScalar, Q, 1, NCOMP, Q, 0>
    (dU + (0 * dstrdU), cstrdU, rU, sTmp, tx); 
    /* then first call (iDIM = 0, iDIMU = 0, iDIMV = 0) */
    magma_grad_3d_device<CeedScalar, 1, 1, NCOMP, Q, P, Q, P, 0, 0, 0>
    (sTinterp, sTgrad, rU, rV, beta, tx, rTmp, sTmp);
    /* there is a sync at the end of magma_grad_3d_device */
    ...
    magma_grad_3d_device<CeedScalar, 1, 1, NCOMP, Q, P, Q, P, 1, 0, 0>
    (sTinterp, sTgrad, rU, rV, beta, tx, rTmp, sTmp);
    /* there is a sync at the end of magma_grad_3d_device */
    ...
    magma_grad_3d_device<CeedScalar, 1, 1, NCOMP, Q, P, Q, P, 2, 0, 0>
    (sTinterp, sTgrad, rU, rV, beta, tx, rTmp, sTmp);
    /* there is a sync at the end of magma_grad_3d_device */

    // write V 
    writeV_3d<CeedScalar, P, 1, NCOMP, P, 0>
    (dV + (0 * dstrdV), cstrdV, rV, tx);
}
