...
- Add even-odd (centro-symmetry) decomposition for tensor contractions, which halves the work when the 1D basis matrix is centro-symmetric.
`CeedBasisGetEvenOddDecompositionInterp1D` and `CeedBasisGetEvenOddDecompositionGrad1D` report the symmetry of a basis and give the cached half-matrices, and `CeedTensorContractApplyEvenOdd` applies a contraction with them, so every CPU backend benefits rather than only `/cpu/self/avx/*`.
