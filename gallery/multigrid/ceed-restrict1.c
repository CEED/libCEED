...
#include "ceed-multigrid1.h"

/**
  @brief Set fields for restriction QFunction that scales inputs for multiplicity
**/
static int CeedQFunctionInit_Restrict1(Ceed ceed, const char *requested,
                                       CeedQFunction qf) {
  int ierr;

  // Check QFunction name
  const char *name = "Restrict1";
...
static void Register(void) {
  CeedQFunctionRegister("Restrict1", Multigrid1_loc, 1, Multigrid1,
                        CeedQFunctionInit_Restrict1);
}
