...
/**
  @brief Ceed QFunction for applying the geometric data for the 2D diff operator
**/
CEED_QFUNCTION(applyDiff2D)(void *ctx, const CeedInt Q,
...
static int CeedQFunctionInit_ApplyDiff2D(Ceed ceed, const char *name,
  ...
  ierr = CeedQFunctionAddInput(qf, "du", 2, CEED_EVAL_GRAD); CeedChk(ierr);
  ierr = CeedQFunctionAddInput(qf, "qdata", 2*(2+1)/2, CEED_EVAL_NONE);
  CeedChk(ierr);
  ierr = CeedQFunctionAddOutput(qf, "dv", 2, CEED_EVAL_GRAD); CeedChk(ierr);
