...
/**
  @brief Set fields for Ceed QFunction applying the geometric data for the 3D
           diff operator
**/
static int CeedQFunctionInit_ApplyDiff3D(Ceed ceed, const char *name,
    CeedQFunction qf) {
  int ierr;

  // Check QFunction name
  if (strcmp(name, "applyDiff3D"))
...
/**
  @brief Register Ceed QFunction for applying the geometric data for the 3D diff
           operator
**/
__attribute__((constructor))
static void Register(void) {
  CeedQFunctionRegister("applyDiff3D", applyDiff3D_loc, 1, applyDiff3D,
                        CeedQFunctionInit_ApplyDiff3D);
}
