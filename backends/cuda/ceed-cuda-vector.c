...
//------------------------------------------------------------------------------
static inline int CeedVectorIsArrayOfType_Cuda(const CeedVector vec,
    CeedMemType mtype, bool *is_array_of_type) {
  int ierr;
  CeedVector_Cuda *data;
  ierr = CeedVectorGetData(vec, &data); CeedChkBackend(ierr);
  ...
  switch (mtype) {
  case CEED_MEM_HOST:
    *is_array_of_type = !!data->h_array_borrowed || !!data->h_array_owned;
    break;
  case CEED_MEM_DEVICE:
    *is_array_of_type = !!data->d_array_borrowed || !!data->d_array_owned;
    break;
  }
