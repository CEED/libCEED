...
static inline int CeedTensorContract_Avx_Blocked(CeedTensorContract contract,
    ...
    float *restrict v, const CeedInt JJ, const CeedInt CC) {
      ...
      for (CeedInt c=0; c<(C/CC)*CC; c+=CC) {
        ...
        for (CeedInt jj=0; jj<JJ; jj++)
          for (CeedInt cc=0; cc<CC/4; cc++)
            vv[jj][cc] = _mm_loadu_ps(&v[(a*J+j+jj)*C+c+cc*4]);
        ...
        for (CeedInt jj=0; jj<JJ; jj++)
          for (CeedInt cc=0; cc<CC/4; cc++)
            _mm_storeu_ps(&v[(a*J+j+jj)*C+c+cc*4], vv[jj][cc]);
      }
    }
    // Remainder of rows
    CeedInt j=(J/JJ)*JJ;
  ...
  for (CeedInt a=0; a<A; a++) {
    ...
    for (CeedInt c = (C/CC)*CC; c<C; c+=4) {
      ...
      for (CeedInt j=0; j<J_break; j+=JJ) {
        ...
        for (CeedInt jj=0; jj<JJ; jj++)
          vv[jj] = _mm_loadu_ps(&v[(a*J+j+jj)*C+c]);

        for (CeedInt b=0; b<B; b++) {
          __m128 tqu;
          if (C-c == 1)
            tqu = _mm_set_ps(0.0, 0.0, 0.0, u[(a*B+b)*C+c+0]);
          else if (C-c == 2)
            tqu = _mm_set_ps(0.0, 0.0, u[(a*B+b)*C+c+1],
                             u[(a*B+b)*C+c+0]);
          else if (C-c == 3)
            tqu = _mm_set_ps(0.0, u[(a*B+b)*C+c+2], u[(a*B+b)*C+c+1],
                             u[(a*B+b)*C+c+0]);
          else
            tqu = _mm_loadu_ps(&u[(a*B+b)*C+c]);
          for (CeedInt jj=0; jj<JJ; jj++) // unroll
            fmadd(vv[jj], tqu, _mm_set1_ps(t[(j+jj)*t_stride_0 + b*t_stride_1]));
        }
        for (CeedInt jj=0; jj<JJ; jj++)
          _mm_storeu_ps(&v[(a*J+j+jj)*C+c], vv[jj]);
      }
    }
...
static inline int CeedTensorContract_Avx_Single(CeedTensorContract contract,
    CeedInt A, CeedInt B, CeedInt C, CeedInt J, const float *restrict t,
    CeedTransposeMode t_mode, const CeedInt add, const float *restrict u,
    float *restrict v, const CeedInt AA, const CeedInt JJ) {
  CeedInt t_stride_0 = B, t_stride_1 = 1;
  if (t_mode == CEED_TRANSPOSE) {
    t_stride_0 = 1; t_stride_1 = J;
  }
    ...
    for (CeedInt j=0; j<(J/JJ)*JJ; j+=JJ) {
      ...
      for (CeedInt aa=0; aa<AA; aa++)
        for (CeedInt jj=0; jj<JJ/4; jj++)
          vv[aa][jj] = _mm_loadu_ps(&v[(a+aa)*J+j+jj*4]);

      for (CeedInt b=0; b<B; b++) {
        for (CeedInt jj=0; jj<JJ/4; jj++) { // unroll
          __m128 tqv = _mm_set_ps(t[(j+jj*4+3)*t_stride_0 + b*t_stride_1],
      ...
      for (CeedInt aa=0; aa<AA; aa++)
        for (CeedInt jj=0; jj<JJ/4; jj++)
          _mm_storeu_ps(&v[(a+aa)*J+j+jj*4], vv[aa][jj]);
    }
  }
  // Remainder of rows
  CeedInt a=(A/AA)*AA;
    ...
    for (CeedInt a=0; a<A_break; a+=AA) {
      ...
      for (CeedInt b=0; b<B; b++) {
        ...
        if (J-j == 1)
          ...
          tqv = _mm_set_ps(0.0, 0.0, t[(j+1)*t_stride_0 + b*t_stride_1],
                           t[(j+0)*t_stride_0 + b*t_stride_1]);
        else if (J-3 == j)
          tqv = _mm_set_ps(0.0, t[(j+2)*t_stride_0 + b*t_stride_1],
                           t[(j+1)*t_stride_0 + b*t_stride_1],
                           t[(j+0)*t_stride_0 + b*t_stride_1]);
        else
          tqv = _mm_set_ps(t[(j+3)*t_stride_0 + b*t_stride_1],
      ...
      for (CeedInt aa=0; aa<AA; aa++)
        _mm_storeu_ps(&v[(a+aa)*J+j], vv[aa]);
    }
  }
  // Remainder of rows, all columns
  for (CeedInt b=0; b<B; b++) {
