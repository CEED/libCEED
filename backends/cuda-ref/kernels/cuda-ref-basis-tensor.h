...
extern "C" __global__ void Interp(const CeedInt num_elem, const int transpose,
  ...
  const CeedInt stride_0 = transpose ? 1 : BASIS_P1D;
  const CeedInt stride_1 = transpose ? BASIS_P1D : 1;
    ...
    for (CeedInt comp = 0; comp < BASIS_NCOMP; ++comp) {
      ...
      for (CeedInt dim1 = 0; dim1 < BASIS_DIM; dim1++) {
        ...
        for (CeedInt dim_2 = 0; dim_2 < BASIS_DIM; dim_2++) {
          ...
          CeedScalar *out = dim_2 == BASIS_DIM - 1
                            ? cur_v
                            : (dim_2 % 2 ? s_buffer_1 : s_buffer_2);
          ...
          for (CeedInt k = i; k < writeLen; k += blockDim.x) {
            ...
            CeedScalar v_k = 0;
            for (CeedInt b = 0; b < P; b++)
              v_k += op[j * stride_0 + b * stride_1] * in[(a * P + b) * post + c];

            if (transpose && dim_2 == BASIS_DIM - 1)
              out[k] += v_k;
            else
              out[k] = v_k;
          }
...
__device__ void Weight1d(const CeedInt num_elem, const CeedScalar *q_weight_1d,
  ...
  if (i < BASIS_Q1D) {
    ...
    if (elem < num_elem)
      w[elem*BASIS_Q1D + i] = q_weight_1d[i];
  }
}
