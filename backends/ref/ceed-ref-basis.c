    ...
    switch (eval_mode) {
      ...
      case CEED_EVAL_INTERP: {
        if (impl->is_collocated) {
          ...
          for (CeedInt d = 0; d < dim; d++) {
            if (interp_symmetry == CEED_SYMMETRY_SYMMETRIC || interp_symmetry == CEED_SYMMETRY_ANTISYMMETRIC) {
      ...
      case CEED_EVAL_GRAD: {
        ...
        if (impl->collo_grad_1d) {
          ...
          for (CeedInt d = 0; d < dim; d++) {
            if (grad_symmetry == CEED_SYMMETRY_SYMMETRIC || grad_symmetry == CEED_SYMMETRY_ANTISYMMETRIC) {
          ...
          for (CeedInt p = 0; p < dim; p++) {
            ...
            for (CeedInt d = 0; d < dim; d++) {
              ...
              if (current_sym == CEED_SYMMETRY_SYMMETRIC || current_sym == CEED_SYMMETRY_ANTISYMMETRIC) {
                CeedCallBackend(
                    CeedTensorContractApplyEvenOdd(contract, pre, P, post, Q, current_t_even, current_t_odd, current_sym, t_mode,
                                                   add && (d == dim - 1), in, out));
              } else {
