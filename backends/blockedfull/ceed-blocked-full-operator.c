...
static int CeedOperatorDestroy_BlockedFull(CeedOperator op) {
  int ierr;
  CeedOperator_BlockedFull *impl;
 ...
 */
static int CeedOperatorSetupFields_BlockedFull(CeedQFunction qf,
  ...
  for (CeedInt i=0; i<numfields; i++) {
    ...
    case CEED_EVAL_INTERP:
      ...
      CeedChk(ierr);
      ierr = CeedVectorCreate(ceed, P*ncomp*blksize, &evecs[i]); CeedChk(ierr);
      ierr = CeedVectorCreate(ceed, Q*ncomp*blksize, &qvecs[i]); CeedChk(ierr);
      break;
    case CEED_EVAL_GRAD:
 ...
 */
static int CeedOperatorSetup_BlockedFull(CeedOperator op) {
  int ierr;
  bool setupdone;
  ...
  // Infields
  ierr = CeedOperatorSetupFields_BlockedFull(qf, op, 0, impl->blkrestr,
                                             impl->evecs, impl->evecsin,
                                             impl->qvecsin, 0,
                                             numinputfields, Q);
  CeedChk(ierr);
  // Outfields
  ierr = CeedOperatorSetupFields_BlockedFull(qf, op, 1, impl->blkrestr,
...
static int CeedOperatorApply_BlockedFull(CeedOperator op, CeedVector invec,
                                         CeedVector outvec,
                                         CeedRequest *request) {
  int ierr;
  CeedOperator_BlockedFull *impl;
  ...
  // Setup
  ierr = CeedOperatorSetup_BlockedFull(op); CeedChk(ierr);
...
int CeedOperatorCreate_BlockedFull(CeedOperator op) {
  int ierr;
  Ceed ceed;
  ierr = CeedOperatorGetCeed(op, &ceed); CeedChk(ierr);
  CeedOperator_BlockedFull *impl;
  ...
  ierr = CeedSetBackendFunction(ceed, "Operator", op, "Apply",
                                CeedOperatorApply_BlockedFull); CeedChk(ierr);
  ierr = CeedSetBackendFunction(ceed, "Operator", op, "Destroy",
                                CeedOperatorDestroy_BlockedFull); CeedChk(ierr);
  return 0;
}
