...
static int CeedOperatorBuildKernelRestriction_Cpu_Gen(std::ostringstream &code, CeedOperator_Cpu_Gen *data, Tab &tab, CeedInt i,
                                                      const CeedInt field_input_buffer[], CeedOperatorField op_field, CeedQFunctionField qf_field,
  ...
  if (is_input) {
    ...
    if (field_input_buffer[i] != i) {
      ...
      if (rstr_type == CEED_RESTRICTION_POINTS) {
        ...
        code << tab << "CeedScalar e_vec" << var_suffix << "[num_comp" << var_suffix << " * max_num_points * block_size];\n";
      ...
      switch (rstr_type) {
        case CEED_RESTRICTION_STRIDED: {
          ...
          code << tab << "\n";
          code << tab << "CeedElemRestriction_Apply_NoTranspose_Strided<block_size, num_comp" << var_suffix << ", elem_size" << var_suffix
        ...
        case CEED_RESTRICTION_STANDARD: {
          ...
          code << tab << "\n";
          code << tab << "CeedElemRestriction_Apply_NoTranspose_Offset<block_size, num_comp" << var_suffix << ", elem_size" << var_suffix
        ...
        case CEED_RESTRICTION_ORIENTED: {
          ...
          code << tab << "constexpr CeedInt comp_stride" << var_suffix << " = " << comp_stride << ";\n";
          code << tab << "\n";
          code << tab << "CeedElemRestriction_Apply_NoTranspose_Oriented<block_size, num_comp" << var_suffix << ", elem_size" << var_suffix
        ...
        case CEED_RESTRICTION_CURL_ORIENTED: {
          ...
          code << tab << "constexpr CeedInt comp_stride" << var_suffix << " = " << comp_stride << ";\n";
          code << tab << "\n";
          code << tab << "CeedElemRestriction_Apply_NoTranspose_CurlOriented<block_size, num_comp" << var_suffix << ", elem_size" << var_suffix
               << ", num_elem, comp_stride" << var_suffix << ">(block, inputs[" << i << "].offsets, inputs[" << i << "].curl_orients, inputs[" << i
               << "].l_vec, e_vec" << var_suffix << ");\n";
          tab.pop();
          code << tab << "}\n";
        } break;
    ...
    switch (rstr_type) {
      case CEED_RESTRICTION_STRIDED: {
        ...
        code << tab << "constexpr CeedInt strides_0 = " << strides[0] << ", strides_1 = " << strides[1] << ", strides_2 = " << strides[2] << ";\n";
        code << tab << "\n";
        code << tab << "CeedElemRestriction_ApplyAdd_Transpose_Strided<block_size, num_comp" << var_suffix << ", elem_size" << var_suffix
      ...
      case CEED_RESTRICTION_STANDARD: {
        ...
        code << tab << "constexpr CeedInt comp_stride" << var_suffix << " = " << comp_stride << ";\n";
        code << tab << "\n";
        code << tab << "CeedElemRestriction_ApplyAdd_Transpose_Offset<block_size, num_comp" << var_suffix << ", elem_size" << var_suffix
      ...
      case CEED_RESTRICTION_ORIENTED: {
        ...
        code << tab << "constexpr CeedInt comp_stride" << var_suffix << " = " << comp_stride << ";\n";
        code << tab << "\n";
        code << tab << "CeedElemRestriction_ApplyAdd_Transpose_Oriented<block_size, num_comp" << var_suffix << ", elem_size" << var_suffix
      ...
      case CEED_RESTRICTION_CURL_ORIENTED: {
        ...
        code << tab << "constexpr CeedInt comp_stride" << var_suffix << " = " << comp_stride << ";\n";
        code << tab << "\n";
        code << tab << "CeedElemRestriction_ApplyAdd_Transpose_CurlOriented<block_size, num_comp" << var_suffix << ", elem_size" << var_suffix
             << ", num_elem, comp_stride" << var_suffix << ">(block, outputs[" << i << "].offsets, outputs[" << i << "].curl_orients, e_vec"
    ...
    if (is_tensor) {
      switch (eval_mode) {
        ...
        case CEED_EVAL_WEIGHT: {
          if (is_at_points) {
            code << tab << "CeedScalar q_vec" << var_suffix << "[max_num_points * block_size];\n";
      ...
      if (eval_mode == CEED_EVAL_WEIGHT) {
        code << tab << "CeedScalar q_vec" << var_suffix << "[" << (is_at_points ? "max_num_points" : "Q") << " * block_size];\n";
...
extern "C" int CeedOperatorBuildKernel_Cpu_Gen(CeedOperator op, bool *is_good_build) {
  ...
  // And best output field order
  bool    output_skip_rstr[CEED_FIELD_MAX] = {false}, output_apply_add_basis[CEED_FIELD_MAX] = {false};
  CeedInt output_field_order[CEED_FIELD_MAX];

  for (CeedInt i = 0; i < num_output_fields; i++) output_field_order[i] = -1;
  {
    bool    is_ordered[CEED_FIELD_MAX];
    CeedInt curr_index = 0;
    ...
    for (CeedInt i = 0; i < num_output_fields; i++) {
      ...
      if (is_ordered[i]) continue;
      is_ordered[i]                  = true;
      output_field_order[curr_index] = i;
      curr_index++;
      ...
      for (CeedInt j = i + 1; j < num_output_fields; j++) {
        ...
        if (rstr_i == rstr_j && vec_i == vec_j) {
          is_ordered[j]                  = true;
          output_field_order[curr_index] = j;
          curr_index++;
        }
        CeedCallBackend(CeedVectorDestroy(&vec_j));
        CeedCallBackend(CeedElemRestrictionDestroy(&rstr_j));
  ...
  for (CeedInt i = num_output_fields - 1; i >= 0; i--) {
    ...
    for (CeedInt j = i - 1; j >= 0; j--) {
      ...
      if (vec_i == vec_j && rstr_i == rstr_j) {
        output_skip_rstr[j]       = true;
        output_apply_add_basis[i] = true;
      }
      CeedCallBackend(CeedVectorDestroy(&vec_j));
      CeedCallBackend(CeedElemRestrictionDestroy(&rstr_j));
    }
  ...
  for (CeedInt i = 0; i < num_input_fields; i++) {
    ...
    CeedCallBackend(CeedOperatorBuildKernelBasis_Cpu_Gen(code, data, tab, field, NULL, op_input_fields[field], qf_input_fields[field], true,
                                                         is_at_points));
  }
  code << tab << "\n";
  ...
  for (CeedInt i = 0; i < num_output_fields; i++) {
    CeedInt field = output_field_order[i];

    CeedCallBackend(CeedOperatorBuildKernelBasis_Cpu_Gen(code, data, tab, field, output_apply_add_basis, op_output_fields[field],
                                                         qf_output_fields[field], false, is_at_points));
    if (!output_skip_rstr[field]) {
      CeedCallBackend(CeedOperatorBuildKernelRestriction_Cpu_Gen(code, data, tab, field, NULL, op_output_fields[field], qf_output_fields[field],
                                                                 false, block_size));
    }
  }
