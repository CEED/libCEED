...
typedef struct { const double* in[16]; double* out[16]; } HipFields_double;
...
inline __device__ void loadMatrix(BackendData& data, const double* d_B, double* B) {
...
CEED_INTERN int CeedHipGenOperatorBuild(CeedOperator op) {
  ...
  code << "  HIP_DYNAMIC_SHARED( double, slice)\n";
