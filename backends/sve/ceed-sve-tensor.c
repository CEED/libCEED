...
#define load_vnum(pg, a, n) svld1_vnum_f64(pg, a, n)
#define store(pg, a, v) svst1_f64(pg, a, v)
#define store_vnum(pg, a, n, v) svst1_vnum_f64(pg, a, n, v)
...
//------------------------------------------------------------------------------
#ifdef CEED_SCALAR_IS_FP64
static inline __attribute__((always_inline)) int CeedTensorContractApplyCore_Sve(CeedTensorContract contract, CeedInt A, CeedInt B, CeedInt C,
                                                                                 CeedInt J, const CeedScalar *restrict t, CeedTransposeMode t_mode,
  ...
  for (CeedInt a = 0; a < A; a++) {
    ...
    while (true) {
      ...
      for (; j + 3 < J; j += 4) {
        ...
        if (add) {
          v_00 = load(pg, &v_a[(CeedSize)j * C + c]);
          v_01 = load(pg, &v_a[(CeedSize)j * C + c_1]);
          v_02 = load(pg, &v_a[(CeedSize)j * C + c_2]);
          v_03 = load(pg_3, &v_a[(CeedSize)j * C + c_3]);
...
#pragma GCC unroll 2
        for (CeedInt b = 0; b < B; b++) {
          ...
          const CeedScalar *u_c      = &u[u_offset + c];
          const rtype       u_0 = load(pg, u_c), u_1 = load_vnum(pg, u_c, 1);
          const rtype       u_2 = load_vnum(pg, u_c, 2), u_3 = load_vnum(pg_3, u_c, 3);
        ...
        CeedScalar *v_0_c = &v_a[(CeedSize)j * C + c];
        CeedScalar *v_1_c = &v_a[(j + 1) * (CeedSize)C + c];
        CeedScalar *v_2_c = &v_a[(j + 2) * (CeedSize)C + c];
        CeedScalar *v_3_c = &v_a[(j + 3) * (CeedSize)C + c];

        store(pg, v_0_c, v_00);
        store_vnum(pg, v_0_c, 1, v_01);
        store_vnum(pg, v_0_c, 2, v_02);
        store_vnum(pg_3, v_0_c, 3, v_03);
        store(pg, v_1_c, v_10);
        store_vnum(pg, v_1_c, 1, v_11);
        store_vnum(pg, v_1_c, 2, v_12);
        store_vnum(pg_3, v_1_c, 3, v_13);
        store(pg, v_2_c, v_20);
        store_vnum(pg, v_2_c, 1, v_21);
        store_vnum(pg, v_2_c, 2, v_22);
        store_vnum(pg_3, v_2_c, 3, v_23);
        store(pg, v_3_c, v_30);
        store_vnum(pg, v_3_c, 1, v_31);
        store_vnum(pg, v_3_c, 2, v_32);
        store_vnum(pg_3, v_3_c, 3, v_33);
      }
#else
      for (; j + 3 < J; j += 4) {
        rtype v_00 = setzero(), v_01 = v_00, v_02 = v_00, v_03 = v_00;
      ...
      for (; j + 1 < J; j += 2) {
        ...
        rtype       v_10 = v_00, v_11 = v_00, v_12 = v_00, v_13 = v_00;
        CeedScalar *v_0_c = &v_a[(CeedSize)j * C + c];
        CeedScalar *v_1_c = v_0_c + C;

        if (add) {
          v_00 = load_vnum(pg, v_0_c, 0);
          v_01 = load_vnum(pg, v_0_c, 1);
          v_02 = load_vnum(pg, v_0_c, 2);
          v_03 = load_vnum(pg_3, v_0_c, 3);
          v_10 = load_vnum(pg, v_1_c, 0);
          v_11 = load_vnum(pg, v_1_c, 1);
          v_12 = load_vnum(pg, v_1_c, 2);
          v_13 = load_vnum(pg_3, v_1_c, 3);
        }
        for (CeedInt b = 0; b < B; b++) {
          const CeedSize    u_offset = ((CeedSize)a * B + b) * C;
          const CeedScalar *u_c      = &u[u_offset + c];
...
#pragma GCC unroll 2
      ...
      for (; j + 1 < J; j += 2) {
        ...
        if (add) {
          v_00 = load(pg, v_0_c);
          v_01 = load(pg_1, v_0_c + vector_length);
          v_10 = load(pg, v_1_c);
          v_11 = load(pg_1, v_1_c + vector_length);
        }
#pragma GCC unroll 2
        for (CeedInt b = 0; b < B; b++) {
          const CeedSize u_offset = ((CeedSize)a * B + b) * C;
        ...
        store(pg, v_0_c, v_00);
        store(pg_1, v_0_c + vector_length, v_01);
        store(pg, v_1_c, v_10);
        store(pg_1, v_1_c + vector_length, v_11);
      }
#else
      for (; j + 1 < J; j += 2) {
        rtype v_00 = setzero(), v_01 = v_00, v_10 = v_00, v_11 = v_00;
...
//------------------------------------------------------------------------------
// Tensor Contract Apply A=1 (FP64)
//------------------------------------------------------------------------------
// Literal A lets the compiler omit the A loop; FP32 measured slower with two instantiations
#ifdef CEED_SCALAR_IS_FP64
static int CeedTensorContractApply_Sve(CeedTensorContract contract, CeedInt A, CeedInt B, CeedInt C, CeedInt J, const CeedScalar *restrict t,
                                       CeedTransposeMode t_mode, const CeedInt add, const CeedScalar *restrict u, CeedScalar *restrict v) {
  if (A == 1) return CeedTensorContractApplyCore_Sve(contract, 1, B, C, J, t, t_mode, add, u, v);
  return CeedTensorContractApplyCore_Sve(contract, A, B, C, J, t, t_mode, add, u, v);
}
#endif
