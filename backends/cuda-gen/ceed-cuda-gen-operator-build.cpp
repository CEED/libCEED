...
inline __device__ void loadMatrix(BackendData &data, const float *__restrict__ d_B, float *B) {
...
//------------------------------------------------------------------------------
template <int Q1d>
inline __device__ void weight1d(BackendData &data, const float *__restrict__ qweight1d, float *w) {
...
//------------------------------------------------------------------------------
template <int Q1d>
inline __device__ void weight2d(BackendData &data, const float *__restrict__ qweight1d, float *w) {
...
//------------------------------------------------------------------------------
template <int Q1d>
inline __device__ void weight3d(BackendData &data, const float *__restrict__ qweight1d, float *w) {
...
extern "C" int CeedCudaGenOperatorBuild(CeedOperator op) {
  ...
  // Setup
  code << "\n// -----------------------------------------------------------------------------\n";
  code << "\nextern \"C\" __global__ void "<<oper<<"(CeedInt nelem, void* ctx, CudaFieldsInt indices, CudaFields fields, CudaFields_float B, CudaFields_float G, float* W) {\n";
  for (CeedInt i = 0; i < numinputfields; i++) {
    ierr = CeedQFunctionFieldGetEvalMode(qfinputfields[i], &emode);
    CeedChkBackend(ierr);
  ...
  code << "  extern __shared__ float slice[];\n";
