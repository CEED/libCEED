                ...
                magma_int_t nelem, magma_queue_t queue)
{
    ...
    if ( (nthreads*ntcol) > nthreads_max || shmem > shmem_max ) {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(magma_interp_2d_kernel<T,NCOMP,P,Q,MAXPQ>), dim3(grid), dim3(threads), shmem, magma_queue_get_hip_stream(queue), dT, transT, dU, estrdU, cstrdU, dV, estrdV, cstrdV, nelem);
