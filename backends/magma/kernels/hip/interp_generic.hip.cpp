    ...
    magma_int_t nelem, magma_queue_t queue)
{
    ...
    if ( nthreads > nthreads_max || shmem > shmem_max ) {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(interp_generic_kernel<T, P, Q>), dim3(grid), dim3(threads), shmem, magma_queue_get_hip_stream(queue),  dim, ncomp, pre, post, tmp_size, dT, transT, 
          dU, estrdU, cstrdU, 
          dV, estrdV, cstrdV );
