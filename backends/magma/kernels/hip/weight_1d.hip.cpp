    ...
    magma_int_t nelem, magma_int_t maxthreads, magma_queue_t queue)
{
    ...
    if ( (nthreads*ntcol) > nthreads_max || shmem > shmem_max ) {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(magma_weight_1d_kernel<T, Q>), dim3(grid), dim3(threads), shmem, magma_queue_get_hip_stream(queue), dqweight1d, dV, v_stride, nelem);
