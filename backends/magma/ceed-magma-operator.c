...
static int CeedOperatorApply_Ref(CeedOperator op, CeedVector invec,
  ...
  for (CeedInt i=0; i<numinputfields; i++) {
    ...
    if (emode == CEED_EVAL_WEIGHT) { // Skip
      ...
      // Get evec
      ierr = CeedVectorGetArrayRead(impl->evecs[i], CEED_MEM_MAGMA,
  ...
  for (CeedInt i=0; i<numoutputfields; i++) {
    ierr = CeedVectorGetArray(impl->evecs[i+impl->numein], CEED_MEM_MAGMA,
  ...
  for (CeedInt e=0; e<numelements; e++) {
    ...
    for (CeedInt i=0; i<numinputfields; i++) {
      ...
      switch(emode) {
      case CEED_EVAL_NONE:
        ierr = CeedVectorSetArray(impl->qvecsin[i], CEED_MEM_MAGMA,
                                  CEED_USE_POINTER,
                                  &impl->edata[i][e*Q*ncomp]); CeedChk(ierr);
        break;
      case CEED_EVAL_INTERP:
        ierr = CeedOperatorFieldGetBasis(opinputfields[i], &basis); CeedChk(ierr);
        ierr = CeedVectorSetArray(impl->evecsin[i], CEED_MEM_MAGMA,
                                  CEED_USE_POINTER,
    ...
    for (CeedInt i=0; i<numoutputfields; i++) {
      ...
      if (emode == CEED_EVAL_NONE) {
        ...
        CeedChk(ierr);
        ierr = CeedVectorSetArray(impl->qvecsout[i], CEED_MEM_MAGMA,
                                  CEED_USE_POINTER,
    ...
    for (CeedInt i=0; i<numoutputfields; i++) {
      ...
      case CEED_EVAL_INTERP:
        ...
        CeedChk(ierr);
        ierr = CeedVectorSetArray(impl->evecsout[i], CEED_MEM_MAGMA,
                                  CEED_USE_POINTER,
