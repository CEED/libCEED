    ...
    magma_int_t nelem, magma_kernel_mode_t kernel_mode, magma_int_t* maxthreads, magma_queue_t queue)
{
    ...
    if(kernel_mode == MAGMA_KERNEL_DIM_SPECIFIC) {
        switch(dim) {
            case 1: launch_failed = magma_weight_1d(Q, dqweight1d, dV, v_stride, nelem, maxthreads[0], queue); break;
            case 2: launch_failed = magma_weight_2d(Q, dqweight1d, dV, v_stride, nelem, maxthreads[1], queue); break;
            case 3: launch_failed = magma_weight_3d(Q, dqweight1d, dV, v_stride, nelem, maxthreads[2], queue); break;
